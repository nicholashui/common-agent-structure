# video.cameraoperator — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.cameraoperator/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.cameraoperator`.
...
|-------|-------|
| **va_id** | 7 |
| **pack_id** | `video.cameraoperator` |
| **upstream_name** | CameraOperatorAgent |
| **category** | `2-Cam` |
| **domain_id** | `video` |
| **previous_common_id** | `video.shot_planner` |
| **status** | `registered` |
...
Host role binding: `CameraOperatorAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
Frame steadiness, focus-hit %, action centering

### Surpass-human signal (historical)

Focus-pull accuracy >99% vs SOC ~97% baseline
...
Runway camera-path presets; Kling motion control API; virtual camera rigs (Unreal MV)

**Runtime safety:** Host allow-lists are only `agent_spec.json` + `tool-permission-register.json`. CI uses video_* stubs. Do not treat design-time vendor names as enabled APIs.

### Architecture pattern (historical)

ReAct (Yao 2022) — reason about framing then call renderer
...
- Prompt reference: `video.prompt.cameraoperator.v1`
- Rubric reference: `video.rubric.cameraoperator.v1`
...
- Pack agent ID `video.cameraoperator` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.shot_planner` is historical mapping only.
...
- Master roster row va_id=7 from embedded agents.md content above.
...
<!-- self_contained_spec · video.cameraoperator · va_id=7 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **CameraOperatorAgent** (`video.cameraoperator`, va_id=7, category `2-Cam`).
...
<!-- migration_capability_research · video.cameraoperator · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.cameraoperator.v1`
...
You are **CameraOperatorAgent (VA Domain Pack)** (`video.cameraoperator`), a pack agent in the video domain swarm.
...
Design tool surface: Runway camera-path presets; Kling motion control API; virtual camera rigs (Unreal MV)
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: CinematographerAgent (per-take feedback)
- May comment on: CinematographerAgent (impractical asks)
...
Criteria (design): Frame steadiness, focus-hit %, action centering
...
Execute the craft step for **CameraOperatorAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.cameraoperator`.
...
### `prompts/video.prompt.cameraoperator.v1.md`
...
Source rubric `video.rubric.cameraoperator.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.cameraoperator.v1.json`
...
# Source acquisition runbook — `video.cameraoperator`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.cameraoperator",
  "plan_id": "video.cameraoperator.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.cameraoperator",
...
### `sources/generic/video.cameraoperator.SPEC.md`
...
```json
{
  ...
  "sources": [
    {
      "kind": "reference_book",
      "language": "EN",
      "title": "The Camera Assistant's Manual, 6th ed.",
      "author": "David E. Elkins",
      "isbn13": "9780240818948",
      "origin": "spec/book_of_knowledge.md",
      "citation": "EN: The Camera Assistant's Manual, 6th ed. (David E. Elkins), ISBN-13 9780240818948"
    },
    {
      "kind": "reference_book",
      "language": "EN",
      ...
      "origin": "spec/book_of_knowledge.md",
      "citation": "EN: Grammar of the Shot, 4th ed., ISBN-13 9781138632219"
    },
    {
      "kind": "reference_book",
      "language": "ZH",
  ...
  "agent_id": "video.cameraoperator",
  "previous_common_agent_id": "video.shot_planner",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.cameraoperator",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:10.553376Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:27.753048+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.cameraoperator",
...
```json
{
  ...
  "sources": [
    ...
    {
      "id": "src_2",
      "title": "Steadicam workshop reels",
      "description": "Steadicam workshop reels",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
    ...
    {
      "id": "src_3",
      "title": "focus-pull telemetry",
      "description": "focus-pull telemetry",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
