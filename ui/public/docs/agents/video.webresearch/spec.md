# video.webresearch — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.webresearch/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.webresearch`.
...
|-------|-------|
| **va_id** | 66 |
| **pack_id** | `video.webresearch` |
| **upstream_name** | WebResearchAgent |
| **category** | `9-Meta` |
| **domain_id** | `video` |
| **previous_common_id** | `video.web_researcher` |
| **status** | `registered` |
...
- Pack agent ID `video.webresearch` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.web_researcher` is historical mapping only.
...
- Master roster row va_id=66 from embedded agents.md content above.
...
<!-- self_contained_spec · video.webresearch · va_id=66 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **WebResearchAgent** (`video.webresearch`, va_id=66, category `9-Meta`).
...
<!-- migration_capability_research · video.webresearch · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.webresearch.v1`
...
You are **WebResearchAgent (VA Domain Pack)** (`video.webresearch`), a pack agent in the video domain swarm.
...
Design tool surface: Brave/Google Search API; Jina Reader (web→markdown); source-quality classifier
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: FactCheckerAgent, CitationAgent
- May comment on: ScriptwriterAgent (uncited claim)
...
Criteria (design): Source-grade per claim; citation precision; recency hit
...
Execute the craft step for **WebResearchAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.webresearch`.
...
### `prompts/video.prompt.webresearch.v1.md`
...
Source rubric `video.rubric.webresearch.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.webresearch.v1",
  "agent_id": "video.webresearch",
  "title": "L2 craft rubric for WebResearchAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Source-grade per claim",
          "description": "Source-grade per claim",
          "weight": 0.3333,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
        },
        {
          ...
          "description": "citation precision",
          "weight": 0.3333,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Faster + more sources than newsroom researcher",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Source-grade per claim; citation precision; recency hit",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.webresearch.v1.json`
...
# Source acquisition runbook — `video.webresearch`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.webresearch",
  "plan_id": "video.webresearch.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.webresearch",
...
### `sources/generic/video.webresearch.SPEC.md`
...
```json
{
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.webresearch",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Bing/Google/Brave APIs",
      "description": "Bing/Google/Brave APIs",
    ...
    {
      "id": "src_3",
      "title": "Perplexity patterns",
      "description": "Perplexity patterns",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
