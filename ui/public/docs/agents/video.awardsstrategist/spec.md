# video.awardsstrategist — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.awardsstrategist/`.
...
# AwardsStrategistAgent

> Self-contained VA Domain Pack agent on host common-agent-swarm-ops.
> Pack agent ID matches pure VA/generic taxonomy: `video.awardsstrategist`.
...
|-------|-------|
| **va_id** | 113 |
| **pack_id** | `video.awardsstrategist` |
| **upstream_name** | AwardsStrategistAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.accessibility_specialist` |
| **status** | `registered` |
...
Host role binding: `AwardsStrategistAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
Submission readiness, category fit, timeline precision

### Surpass-human signal (historical)

Improves awards-timing discipline over generic release planning
...
Awards calendars, campaign trackers, submission checklists

**Runtime safety:** Host allow-lists are only `agent_spec.json` + `tool-permission-register.json`. CI uses video_* stubs. Do not treat design-time vendor names as enabled APIs.

### Architecture pattern (historical)

ReAct with awards timeline optimization
...
- Prompt reference: `video.prompt.awardsstrategist.v1`
- Rubric reference: `video.rubric.awardsstrategist.v1`
...
- Pack agent ID `video.awardsstrategist` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.accessibility_specialist` is historical mapping only.
...
- Master roster row va_id=113 from embedded agents.md content above.
...
<!-- self_contained_spec · video.awardsstrategist · va_id=113 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **AwardsStrategistAgent** (`video.awardsstrategist`, va_id=113, category `10-Sup`).
...
<!-- migration_capability_research · video.awardsstrategist · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.awardsstrategist.v1`
...
You are **AwardsStrategistAgent (VA Domain Pack)** (`video.awardsstrategist`), a pack agent in the video domain swarm.
...
Primary distillation sources (design): Awards calendars, campaign playbooks, category positioning histories

## Developer

### Tools (allowlist intent)
Design tool surface: Awards calendars, campaign trackers, submission checklists
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: ProducerAgent / EP, CriticAgent, MarketingAgent
- May comment on: Weak campaign timing, poor category fit, incomplete submission assets
...
Criteria (design): Submission readiness, category fit, timeline precision
...
Execute the craft step for **AwardsStrategistAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.awardsstrategist`.
...
### `prompts/video.prompt.awardsstrategist.v1.md`
...
Source rubric `video.rubric.awardsstrategist.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.awardsstrategist.v1.json`
...
# Source acquisition runbook — `video.awardsstrategist`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.awardsstrategist",
  "plan_id": "video.awardsstrategist.distill.v1",
  "inputs": [
    "src_1"
  ],
  "extractors": [
  ...
  "memory_namespace": "pack.video.video.awardsstrategist",
...
```json
{
  ...
  "agent_id": "video.awardsstrategist",
  "previous_common_agent_id": "video.accessibility_specialist",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.awardsstrategist",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:11.048366Z",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Awards calendars, campaign playbooks, category positioning histories",
      "description": "Awards calendars, campaign playbooks, category positioning histories",
