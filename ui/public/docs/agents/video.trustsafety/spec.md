# video.trustsafety — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.trustsafety/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.trustsafety`.
...
| **upstream_name** | TrustSafetyAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.privacy_reviewer` |
| **status** | `registered` |
...
- Pack agent ID `video.trustsafety` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.privacy_reviewer` is historical mapping only.
...
# Prompt — `video.prompt.trustsafety.v1`
...
You are **TrustSafetyAgent (VA Domain Pack)** (`video.trustsafety`), a pack agent in the video domain swarm.
...
Design tool surface: Safety classifiers, abuse taxonomy DB, moderation APIs
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: ComplianceAgent, DeepfakeDetectionAgent, SafetyRedTeamAgent
- May comment on: Harmful misuse pathways, impersonation vectors, policy gaps
...
Criteria (design): Policy hit rate, abuse-risk recall, low false negatives on blocked cases
...
Execute the craft step for **TrustSafetyAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.trustsafety`.
...
### `prompts/video.prompt.trustsafety.v1.md`
...
Source rubric `video.rubric.trustsafety.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.trustsafety.v1",
  "agent_id": "video.trustsafety",
  "title": "L2 craft rubric for TrustSafetyAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Policy hit rate, abuse-risk recall, low false negatives on blocked cases",
          "description": "Policy hit rate, abuse-risk recall, low false negatives on blocked cases",
          "weight": 1.0,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Catches misuse risk earlier than generic moderation queues",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Policy hit rate, abuse-risk recall, low false negatives on blocked cases",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.trustsafety.v1.json`
...
# Source acquisition runbook — `video.trustsafety`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.trustsafety",
  "plan_id": "video.trustsafety.distill.v1",
  "inputs": [
    "src_1"
  ],
  "extractors": [
  ...
  "memory_namespace": "pack.video.video.trustsafety",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Abuse-taxonomy corpora, impersonation cases, policy rulebooks",
      "description": "Abuse-taxonomy corpora, impersonation cases, policy rulebooks",
