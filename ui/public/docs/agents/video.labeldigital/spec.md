# video.labeldigital — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.labeldigital/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.labeldigital`.
...
|-------|-------|
| **va_id** | 102 |
| **pack_id** | `video.labeldigital` |
| **upstream_name** | LabelDigitalAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.archive_manager` |
| **status** | `registered` |
...
Host role binding: `LabelDigitalAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
Metadata completeness, rollout timing, channel readiness

### Surpass-human signal (historical)

Delivers cleaner label-side packages than ad hoc release ops
...
Digital release systems, channel dashboards, metadata tools

**Runtime safety:** Host allow-lists are only `agent_spec.json` + `tool-permission-register.json`. CI uses video_* stubs. Do not treat design-time vendor names as enabled APIs.

### Architecture pattern (historical)

ReAct on release package requirements
...
- Prompt reference: `video.prompt.labeldigital.v1`
- Rubric reference: `video.rubric.labeldigital.v1`
...
- Pack agent ID `video.labeldigital` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.archive_manager` is historical mapping only.
...
- Master roster row va_id=102 from embedded agents.md content above.
...
<!-- self_contained_spec · video.labeldigital · va_id=102 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **LabelDigitalAgent** (`video.labeldigital`, va_id=102, category `10-Sup`).
...
<!-- migration_capability_research · video.labeldigital · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.labeldigital.v1`
...
You are **LabelDigitalAgent (VA Domain Pack)** (`video.labeldigital`), a pack agent in the video domain swarm.
...
Design tool surface: Digital release systems, channel dashboards, metadata tools
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: MusicVideoDirectorAgent, SocialMediaStrategistAgent, MarketingAgent
- May comment on: Missing metadata, release timing issues, asset-version confusion
...
Criteria (design): Metadata completeness, rollout timing, channel readiness
...
Execute the craft step for **LabelDigitalAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.labeldigital`.
...
### `prompts/video.prompt.labeldigital.v1.md`
...
Source rubric `video.rubric.labeldigital.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.labeldigital.v1.json`
...
# Source acquisition runbook — `video.labeldigital`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.labeldigital",
  "plan_id": "video.labeldigital.distill.v1",
  "inputs": [
    "src_1"
  ],
  "extractors": [
  ...
  "memory_namespace": "pack.video.video.labeldigital",
...
### `sources/generic/video.archivemaster.SPEC.md`
...
### `sources/generic/video.archiveproducer.SPEC.md`
...
```json
{
  ...
  "agent_id": "video.labeldigital",
  "previous_common_agent_id": "video.archive_manager",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.labeldigital",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:11.192086Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:28.298679+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "vendor/common-agent-swarm-ops/business/video/agents/video.labeldigital",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Digital music release operations, metadata schemas, distribution platform requirements",
      "description": "Digital music release operations, metadata schemas, distribution platform requirements",
