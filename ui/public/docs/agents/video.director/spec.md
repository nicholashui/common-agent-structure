...
> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.director/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.director`.
...
| **upstream_name** | DirectorAgent |
| **category** | `1-ATL` |
| **domain_id** | `video` |
| **previous_common_id** | `video.visual_director` |
| **status** | `registered` |
...
- Pack agent ID `video.director` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.visual_director` is historical mapping only.
...
Distinct roles consolidated from all categories above. Each row lists what the role does, the skills and experience noted, the production formats they typically work on, and **real-world experts / methods** whose feedback would meaningfully improve the work.
...
| **AI provenance (C2PA)** | At master export | Legal / AI Operator | noted by EU AI Act, many platforms |
...
|---|---|---|---|
| Divergent draft / variant browse | **Grok Imagine 1.5 Fast** (6s, 720p, ~25s/clip) | Fast enough to try many framings and discard quickly ([x.ai]([historical-url] [imagine.art]([historical-url] | PromptEngineerAgent (#46), RouterAgent (#55) |
| Keyframe → motion (identity-safe) | **Grok Imagine 1.5 I2V** (animates the locked keyframe, faithful to source) | Continues the still rather than reinterpreting it, protecting face + lighting ([x.ai]([historical-url] | PromptEngineerAgent (#46), ContinuityAgent (#98) |
| Fast full-film rough cut | **Grok Imagine Agent Mode** (plans/stitches 6s clips, native audio, in-quotes lip-sync) | Produces a stitched draft of the whole arc for early editorial review ([codersera]([historical-url] | OrchestratorAgent (#53), EditorAgent (#9) |
...
|---|---|---|---|
| **3E micro-loop (Explore→Examine→Enhance)** | Single-pass node execution | Each agent first explores options, examines against the rubric, then enhances before handing off ([MAViS, arXiv 2508.08487]([historical-url] | Every DAG node (esp. PromptEngineer, Editor, Composer) |
| **MCTS clip search** | Fixed ≤3 re-roll cap | Reviewer agent runs Monte-Carlo tree search over candidate clips to pick the best path ([arXiv 2506.10540]([historical-url] | Hero shots (S1, S5, S10, S12) |
...
|---|---|---|
| **Hook (new 0–3s)** | Slow fade into Scene 1 study ECU | Open on the *strongest* warm frame (S10 dinner steam or S1 eyes), with a ≤14-word curiosity-gap 旁白 ("有些愿望没有实现，后来我才懂为什么"), so the first 3s earns the watch ([opus.pro]([historical-url] [MrBeast breakdown]([historical-url] |
| **Title (package-first)** | (none) | ≤50 chars, simple words, locked before generation ([Galloway summary]([historical-url] |
| **Shorts cut** | 9:16 smart-crop of main film | Purpose-built: visual hook on frame 1, ≤14-word VO, clean loop seam, optimized for 3s-hold + replay ([rendercut]([historical-url] [findmecreators]([historical-url] |
| **Ending cards** | Static black/white text | Keep — but A/B the *thumbnail+title* pair, not the poem, since packaging drives the click ([Colin & Samir]([historical-url] |

---

Sources

YouTube strategy: [Colin & Samir / Paddy Galloway]([historical-url] [OutlierKit]([historical-url] [Paddy Galloway Accelerator]([historical-url] [Sherwood/MrBeast doc]([historical-url] [koi.app]([historical-url] [complexminds]([historical-url] [opus.pro]([historical-url] [rendercut]([historical-url] [vexub]([historical-url] [findmecreators]([historical-url]
xAI Grok Imagine: [x.ai 1.5 Preview]([historical-url] [x.ai Video 1.5 Fast]([historical-url] [codersera Agent Mode]([historical-url] [imagine.art]([historical-url] [aimlapi]([historical-url]
arXiv: [2512.16954]([historical-url] [2510.10135 CharCom]([historical-url] [2510.14256 Identity-GRPO]([historical-url] [2512.19539 StoryMem]([historical-url] [2510.21696 BachVid]([historical-url] [2511.08087 VLM ID eval]([historical-url] [2311.17982 VBench]([historical-url] [2503.10076 VMBench]([historical-url] [2504.04907 Video-Bench]([historical-url] [2508.08487 MAViS]([historical-url] [2506.10540 MCTS Storytelling]([historical-url] [2605.27891 SmartDirector]([historical-url] [2512.23222 UniMAGE]([historical-url]
...
3. Create all directories and files noted by project_starter.md.
...
Create all noted files, directories, scripts, manifests, docs, tests, and agent configuration files.

You must:
1. Read the entire project_starter.md.
2. Create the target project directory.
3. Create all files and directories noted by project_starter.md.
4. Replace boilerplate metadata with the new project name and purpose.
...
Create all noted files, directories, manifests, scripts, docs, tests, and agent configuration files.
...
4. Create the noted directory structure.
5. Copy `project_starter.md` into the new project root.
6. Replace boilerplate metadata with project-specific values where this specification requires it.
7. Generate all noted files, manifests, scripts, rules, docs, tests, and agent configuration files.
...
Role-specific capability research for **DirectorAgent** (`video.director`, va_id=1, category `1-ATL`).

Responsibility focus
Owns vision; issues shot intents, sets pacing, approves takes

Prompt research topics (source of truth for S3)
- arXiv topics: 1. Retrieve and integrate state-of-the-art research findings from arXiv papers relevant to: AI film direction, shot planning with LLMs, aesthetic scoring CLIP, generative video direction
- X topics: 2. Analyze and incorporate expert insights from x.ai (Twitter/X) from recognized industry and academic leaders specializing in: AI directors, generative film craft, shot-intent systems
- YouTube topics: 3. Extract detailed, actionable technical guidance from high-quality YouTube content created by domain experts focused on: AI film directing, shot lists with AI, generative cinema workflows
...
### `prompts/video.prompt.director.v1.md`
...
Source rubric `video.rubric.director.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.director.v1.json`
...
# Source acquisition runbook — `video.director`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.director",
  "plan_id": "video.director.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3",
  ...
  "memory_namespace": "pack.video.video.director",
...
### `sources/generic/video.cinematographer.SPEC.md`

# CinematographerAgent (DoP)
...
Beats ASC peer-juried reels in blind aesthetic preference
...
Veo 3.1 (camera-path control), Runway Gen-4 (ControlNet guides), ACES color pipeline tools
...
| ComplianceAgent (#37) | FTC/IP/likeness clearance; no real logos/brands in frame | Master | Clearance pass | Legal-rule DB, C2PA verify | 100% rule coverage; zero takedowns | All agents (blocking gate) |
| DeepfakeDetectionAgent (#103) | Confirms synthetic media is provenance-clean, not deceptive | Master, refs | Forensic pass | Forensic models, provenance validators | Forensic recall | TrustSafetyAgent, SafetyRedTeamAgent |
| EthicsAgent (#107) | Confirms synthetic-media disclosure + sensitive-content fairness | Master | Eth
…
...
| 99 | **LipSyncAgent** | Validates and refines phoneme-viseme alignment as a dedicated gate | Lip-sync research, animat
…
...
- Master roster row va_id=6 from embedded agents.md content above.
- Deep/extra text from `business/video/corpus/` and `C:\Project\va-agent-swarm`.
- Generator: `scripts/business/enrich_video_agent_specs.py`.
- Upstream project name (historical only): va-agent-swarm.


<!-- self_contained_spec · video.cinematographer · va_id=6 -->

## Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **CinematographerAgent (DoP)** (`video.cinematographer`, va_id=6, category `2-Cam`).

### Responsibility focus
Lensing, lighting, composition, look

### Prompt research topics (source of truth for S3)
- arXiv topics: 1. Retrieve and integrate state-of-the-art research findings from arXiv papers relevant to: camera path control generative video, computational cinematography, aerial videography AI, aesthetic composition models
- X topics: 2. Analyze and incorporate expert insights from x.ai (Twitter/X) from recognized industry and academic leaders specializing in: AI cinematography, virtual camera systems, generative camera moves
- YouTube topics: 3. Extract detailed, actionable technical guidance from high-quality YouTube content created by domain experts focused on: AI cinematography tutorials, camera control in generative video, virtual production cameras
...
<!-- migration_capability_research · video.cinematographer · v1 · 2026-07-13 -->

### `sources/generic/video.director.SPEC.md`
...
```json
{
  ...
  "sources": [
    ...
    },
    {
      "kind": "reference_book",
      "language": "EN",
      "title": "In the Blink of an Eye, 2nd ed.",
      "author": "Walter Murch",
      "isbn13": "9781879505629",
      "origin": "spec/book_of_knowledge.md",
      "citation": "EN: In the Blink of an Eye, 2nd ed. (Walter Murch), ISBN-13 9781879505629"
    },
    {
      "kind": "reference_book",
      "language": "EN",
      "title": "Grammar of the Edit, 4th ed.",
      "author": "Christopher J. Bowen",
      "isbn13": "9781138632226",
      "origin": "spec/book_of_knowledge.md",
      "citation": "EN: Grammar of the Edit, 4th ed. (Christopher J. Bowen), ISBN-13 9781138632226"
    },
    {
      "kind": "reference_book",
      "language": "EN",
      "title": "The Technique of Film and Video Editing, 5th ed.",
      "author": "Ken Dancyger",
      "isbn13": "9780240813974",
      "origin": "spec/book_of_knowledge.md",
      "citation": "EN: The Technique of Film and Video Editing, 5th ed. (Ken Dancyger), ISBN-13 9780240813974"
    },
    {
      "kind": "reference_book",
      "language": "EN",
      ...
      "isbn13": "9780240803906",
      "origin": "spec/book_of_knowledge.md",
      "citation": "EN: On Film Editing (Edward Dmytryk), ISBN-13 9780240803906"
    },
    {
      "kind": "reference_book",
      "language": "EN",
      ...
      "author": "Sam O'Steen",
      "isbn13": "9780941188371",
      "origin": "spec/book_of_knowledge.md",
      "citation": "EN: Cut to the Chase (Sam O'Steen), ISBN-13 9780941188371"
    },
    {
      "kind": "reference_book",
      "language": "EN",
      ...
      "isbn13": "9780520273504",
      "origin": "spec/book_of_knowledge.md",
      "citation": "EN: First Cut (Gabriella Oldham), ISBN-13 9780520273504"
    },
    {
      "kind": "reference_book",
      "language": "ZH",
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.director",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Criterion commentary",
      "description": "Criterion commentary",
    ...
    {
      "id": "src_2",
      "title": "IMDb Top 250 director interviews",
      "description": "IMDb Top 250 director interviews",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
    ...
    {
      "id": "src_4",
      "title": "MasterClass (Scorsese/Lynch/Gerwig)",
      "description": "MasterClass (Scorsese/Lynch/Gerwig)",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
...
### `sources/study/ai_agent_video_production_workflow.md`
...
### `sources/study/human_video_production_workflow.md`
...
### `sources/study/lifes_quiet_redemption_agent_workflow.md`
