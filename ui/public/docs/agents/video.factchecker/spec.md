# video.factchecker — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.factchecker/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.factchecker`.
...
|-------|-------|
| **va_id** | 34 |
| **pack_id** | `video.factchecker` |
| **upstream_name** | FactCheckerAgent |
| **category** | `7-Edu` |
| **domain_id** | `video` |
| **previous_common_id** | `video.factual_verifier` |
| **status** | `registered` |
...
Host role binding: `FactCheckerAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
Source-grade per claim (primary > secondary); cross-source ≥2

### Surpass-human signal (historical)

Lower correction rate than Pulitzer-tier outlets
...
Web search APIs (Brave/Google); claim-extraction NER; source-quality classifier

**Runtime safety:** Host allow-lists are only `agent_spec.json` + `tool-permission-register.json`. CI uses video_* stubs. Do not treat design-time vendor names as enabled APIs.

### Architecture pattern (historical)

ReAct (extract claim → search → verify → grade)
...
- Prompt reference: `video.prompt.factchecker.v1`
- Rubric reference: `video.rubric.factchecker.v1`
...
- Pack agent ID `video.factchecker` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.factual_verifier` is historical mapping only.
...
| Post | EditorAgent + VoiceOverAgent + ColoristAgent +
…
...
- Master roster row va_id=34 from embedded agents.md content above.
...
<!-- self_contained_spec · video.factchecker · va_id=34 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **FactCheckerAgent** (`video.factchecker`, va_id=34, category `7-Edu`).
...
<!-- migration_capability_research · video.factchecker · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.factchecker.v1`
...
You are **FactCheckerAgent (VA Domain Pack)** (`video.factchecker`), a pack agent in the video domain swarm.
...
Design tool surface: Web search APIs (Brave/Google); claim-extraction NER; source-quality classifier
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: SMEAgent, StandardsEditorAgent
- May comment on: ScriptwriterAgent (unsourced), JournalistAgent
...
Criteria (design): Source-grade per claim (primary > secondary); cross-source ≥2
...
Execute the craft step for **FactCheckerAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.factchecker`.
...
### `prompts/video.prompt.factchecker.v1.md`
...
Source rubric `video.rubric.factchecker.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.factchecker.v1",
  "agent_id": "video.factchecker",
  "title": "L2 craft rubric for FactCheckerAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Source-grade per claim (primary > secondary)",
          "description": "Source-grade per claim (primary > secondary)",
          "weight": 0.5,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Lower correction rate than Pulitzer-tier outlets",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Source-grade per claim (primary > secondary); cross-source ≥2",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.factchecker.v1.json`
...
# Source acquisition runbook — `video.factchecker`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.factchecker",
  "plan_id": "video.factchecker.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.factchecker",
...
### `sources/generic/video.citation.SPEC.md`
...
### `sources/generic/video.factchecker.SPEC.md`
...
```json
{
  ...
  "agent_id": "video.factchecker",
  "previous_common_agent_id": "video.factual_verifier",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.factchecker",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:10.750449Z",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "New Yorker fact-check handbook",
      "description": "New Yorker fact-check handbook",
    ...
    {
      "id": "src_3",
      "title": "Snopes/PolitiFact",
      "description": "Snopes/PolitiFact",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
