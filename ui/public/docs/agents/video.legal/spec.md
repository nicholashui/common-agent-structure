...
> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.legal/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.legal`.
...
| **va_id** | 93 |
| **pack_id** | `video.legal` |
| **upstream_name** | LegalAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.rights_consent_agent` |
| **status** | `registered` |
...
- Pack agent ID `video.legal` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.rights_consent_agent` is historical mapping only.
...
You are **LegalAgent (VA Domain Pack)** (`video.legal`), a pack agent in the video domain swarm.
...
Design tool surface: Legal memo systems, rights trackers, clearance databases
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: ComplianceAgent (Legal), JournalistAgent, ProducerAgent / EP, MPAAgent
- May comment on: Novel legal risks, unclear rights, unresolved high-risk claims
...
Criteria (design): Issue identification recall, sign-off completeness, escalation quality
...
Execute the craft step for **LegalAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.legal`.
...
### `prompts/video.prompt.legal.v1.md`
...
Source rubric `video.rubric.legal.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.legal.v1.json`
...
# Source acquisition runbook — `video.legal`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.legal",
  "plan_id": "video.legal.distill.v1",
  "inputs": [
    "src_1"
  ],
  "extractors": [
  ...
  "memory_namespace": "pack.video.video.legal",
...
```json
{
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.legal",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Media law references, clearance workflows, defamation/IP/privacy cases",
      "description": "Media law references, clearance workflows, defamation/IP/privacy cases",
