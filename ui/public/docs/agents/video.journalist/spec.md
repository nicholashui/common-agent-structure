# video.journalist — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.journalist/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.journalist`.
...
|-------|-------|
| **va_id** | 36 |
| **pack_id** | `video.journalist` |
| **upstream_name** | JournalistAgent |
| **category** | `7-Edu` |
| **domain_id** | `video` |
| **previous_common_id** | `video.production_coordinator` |
| **status** | `registered` |
...
Host role binding: `JournalistAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
Source diversity; on-record ratio; ethical-checklist pass

### Surpass-human signal (historical)

Lower correction rate + faster file vs newsroom
...
Reflexion (ethical-checklist as verbal feedback)
...
- Prompt reference: `video.prompt.journalist.v1`
- Rubric reference: `video.rubric.journalist.v1`
...
- Pack agent ID `video.journalist` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.production_coordinator` is historical mapping only.
...
- Master roster row va_id=36 from embedded agents.md content above.
...
<!-- self_contained_spec · video.journalist · va_id=36 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **JournalistAgent** (`video.journalist`, va_id=36, category `7-Edu`).
...
<!-- migration_capability_research · video.journalist · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.journalist.v1`
...
You are **JournalistAgent (VA Domain Pack)** (`video.journalist`), a pack agent in the video domain swarm.
...
Primary distillation sources (design): Pulitzer/duPont/Peabody winners; SPJ Ethics; Poynter

## Developer

### Tools (allowlist intent)
Design tool surface: Web research tools; AP Stylebook API; interview transcription (Otter); SPJ rubric
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: FactCheckerAgent, LegalAgent, StandardsEditorAgent
- May comment on: FactCheckerAgent, ScriptwriterAgent
...
Criteria (design): Source diversity; on-record ratio; ethical-checklist pass
...
Execute the craft step for **JournalistAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.journalist`.
...
### `prompts/video.prompt.journalist.v1.md`
...
Source rubric `video.rubric.journalist.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.journalist.v1",
  "agent_id": "video.journalist",
  "title": "L2 craft rubric for JournalistAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        ...
        {
          "id": "d2",
          "name": "on-record ratio",
          "description": "on-record ratio",
          "weight": 0.3333,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
        ...
        {
          "id": "d3",
          "name": "ethical-checklist pass",
          "description": "ethical-checklist pass",
          "weight": 0.3334,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Lower correction rate + faster file vs newsroom",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Source diversity; on-record ratio; ethical-checklist pass",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.journalist.v1.json`
...
# Source acquisition runbook — `video.journalist`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.journalist",
  "plan_id": "video.journalist.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.journalist",
...
```json
{
  ...
  "agent_id": "video.journalist",
  "previous_common_agent_id": "video.production_coordinator",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.journalist",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:11.182175Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:28.258361+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.journalist",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Pulitzer/duPont/Peabody winners",
      "description": "Pulitzer/duPont/Peabody winners",
