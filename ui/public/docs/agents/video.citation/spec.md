...
> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.citation/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.citation`.
...
| **upstream_name** | CitationAgent |
| **category** | `9-Meta` |
| **domain_id** | `video` |
| **previous_common_id** | `video.provenance_agent` |
| **status** | `registered` |
...
Host role binding: `CitationAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
- Prompt reference: `video.prompt.citation.v1`
- Rubric reference: `video.rubric.citation.v1`
...
- Pack agent ID `video.citation` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.provenance_agent` is historical mapping only.
...
# Prompt — `video.prompt.citation.v1`
...
You are **CitationAgent (VA Domain Pack)** (`video.citation`), a pack agent in the video domain swarm.
...
Primary distillation sources (design): Chicago, APA, AP style; SPJ grading; CRAAP test

## Developer

### Tools (allowlist intent)
Design tool surface: Citation parsers (AnyStyle); DOI resolver; CRAAP scoring model
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: FactCheckerAgent, JournalistAgent
- May comment on: WebResearchAgent (weak source)
...
Criteria (design): Citation format 100% valid; primary % ≥target
...
Execute the craft step for **CitationAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.citation`.
...
### `prompts/video.prompt.citation.v1.md`
...
Source rubric `video.rubric.citation.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.citation.v1.json`
...
# Source acquisition runbook — `video.citation`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.citation",
  "plan_id": "video.citation.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.citation",
...
### `sources/generic/video.deepfakedetection.SPEC.md`
...
Role-specific capability research for **DeepfakeDetectionAgent** (`video.deepfakedetection`, va_id=103, category `10-Sup`).
...
<!-- migration_capability_research · video.deepfakedetection · v1 · 2026-07-13 -->
...
```json
{
  ...
  "agent_id": "video.citation",
  "previous_common_agent_id": "video.provenance_agent",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.citation",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:10.825479Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:27.837957+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.citation",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Chicago, APA, AP style",
      "description": "Chicago, APA, AP style",
