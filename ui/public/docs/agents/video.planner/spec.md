...
> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.planner/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.planner`.
...
| **upstream_name** | PlannerAgent |
| **category** | `9-Meta` |
| **domain_id** | `video` |
| **previous_common_id** | `video.brief_intake` |
| **status** | `registered` |
...
- Pack agent ID `video.planner` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.brief_intake` is historical mapping only.
...
- Optional Knowledge Graph traversal (e.g., "find all requirements that trace to this component or references it").
...
Each milestone below specifies: **Goal · references · Build (files) · Claude Code workflow · Tests · Acceptance Gate**. Treat the Acceptance Gate as a hard stop — do not start the next milestone until it is green and logged in `BUILD_PROGRESS.md`.
...
# Prompt — `video.prompt.planner.v1`
...
You are **PlannerAgent (VA Domain Pack)** (`video.planner`), a pack agent in the video domain swarm.
...
Primary distillation sources (design): PMBOK; CrewAI task graphs; phase templates

## Developer

### Tools (allowlist intent)
Design tool surface: LangGraph plan-gen; cost-estimation models; Gantt/PERT tools
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: ProducerAgent, FinanceAgent (budget)
- May comment on: RouterAgent (wrong pick), OrchestratorAgent
...
Criteria (design): Plan validity (no missing gate); cost variance <10%
...
Execute the craft step for **PlannerAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.planner`.
...
### `prompts/video.prompt.planner.v1.md`
...
Source rubric `video.rubric.planner.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.planner.v1.json`
...
# Source acquisition runbook — `video.planner`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.planner",
  "plan_id": "video.planner.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.planner",
...
```json
{
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.planner",
...
```json
{
  ...
  "sources": [
    ...
    {
      "id": "src_2",
      "title": "CrewAI task graphs",
      "description": "CrewAI task graphs",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
...
### `sources/study/system_build_plan.md`
