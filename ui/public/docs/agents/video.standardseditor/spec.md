# video.standardseditor — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.standardseditor/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.standardseditor`.
...
| **upstream_name** | StandardsEditorAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.script_editor` |
| **status** | `registered` |
...
Host role binding: `StandardsEditorAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
- Prompt reference: `video.prompt.standardseditor.v1`
- Rubric reference: `video.rubric.standardseditor.v1`
...
- Pack agent ID `video.standardseditor` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.script_editor` is historical mapping only.
...
# Prompt — `video.prompt.standardseditor.v1`
...
You are **StandardsEditorAgent (VA Domain Pack)** (`video.standardseditor`), a pack agent in the video domain swarm.
...
Design tool surface: Editorial checklists, attribution validators, standards DB
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: JournalistAgent, FactCheckerAgent, CorrectionsAgent, LegalAgent
- May comment on: Weak attribution, standards violations, correction policy gaps
...
Criteria (design): Standards-compliance rate, attribution accuracy, corrections readiness
...
Execute the craft step for **StandardsEditorAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.standardseditor`.
> Does **not** enable production models or network. Host `agent_spec.json` remains authoritative.

### Applied item ids
14, 15, 20, 21, 26, 30, 31, 35, 37, 38, 40, 43, 48, 49, 50, 59, 63, 77, 86, 87, 88, 89, 90, 91, 93, 94, 95
...
### `prompts/video.prompt.standardseditor.v1.md`
...
Source rubric `video.rubric.standardseditor.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.standardseditor.v1.json`
...
# Source acquisition runbook — `video.standardseditor`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.standardseditor",
  "plan_id": "video.standardseditor.distill.v1",
  "inputs": [
    "src_1"
  ],
  "extractors": [
  ...
  "memory_namespace": "pack.video.video.standardseditor",
...
### `sources/generic/video.screenwriter.SPEC.md`
...
```json
{
  ...
  "agent_id": "video.standardseditor",
  "previous_common_agent_id": "video.script_editor",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.standardseditor",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:10.531751Z",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Newsroom standards manuals, corrections policies, attribution standards",
      "description": "Newsroom standards manuals, corrections policies, attribution standards",
