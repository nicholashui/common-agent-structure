# video.orchestrator — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.orchestrator/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.orchestrator`.
...
| **upstream_name** | OrchestratorAgent |
| **category** | `9-Meta` |
| **domain_id** | `video` |
| **previous_common_id** | `video.orchestrator` |
| **status** | `registered` |
...
- Pack agent ID `video.orchestrator` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.orchestrator` is historical mapping only.
...
**references:** M0 (contracts), M1 (so nodes can call RAG).
...
**references:** M3 (events exist); can **start in parallel after M3 in a git worktree** against a fake-event emitter, then integrate.
...
88. `make verify` green at every commit; CI noted checks enforced.
...
You are **OrchestratorAgent (VA Domain Pack)** (`video.orchestrator`), a pack agent in the video domain swarm.
...
Design tool surface: LangGraph state machine; Temporal workflow engine; Redis (distributed locks); observability (LangSmith)
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: ProducerAgent (scope), JudgeAgent (dispute), HiTL on stall
- May comment on: All agents (resource burn, retry storms)
...
Criteria (design): DAG completion ≥99.5%; SLA adherence; deadlock = 0
...
Execute the craft step for **OrchestratorAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.orchestrator`.
...
### `prompts/video.prompt.orchestrator.v1.md`
...
Source rubric `video.rubric.orchestrator.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.orchestrator.v1",
  "agent_id": "video.orchestrator",
  "title": "L2 craft rubric for OrchestratorAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "DAG completion ≥99.5%",
          "description": "DAG completion ≥99.5%",
          "weight": 0.3333,
          "threshold_hint": "≥99.5%",
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Lower TTD than human EP at same scope",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "DAG completion ≥99.5%; SLA adherence; deadlock = 0",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.orchestrator.v1.json`
...
# Source acquisition runbook — `video.orchestrator`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.orchestrator",
  "plan_id": "video.orchestrator.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.orchestrator",
...
```json
{
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.orchestrator",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "LangGraph + CrewAI + AutoGen patterns",
      "description": "LangGraph + CrewAI + AutoGen patterns",
    ...
    {
      "id": "src_3",
      "title": "PGA schedule templates",
      "description": "PGA schedule templates",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
