# video.distributor — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.distributor/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.distributor`.
...
| **upstream_name** | DistributorAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.delivery_qc_reviewer` |
| **status** | `registered` |
...
Host role binding: `DistributorAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
- Prompt reference: `video.prompt.distributor.v1`
- Rubric reference: `video.rubric.distributor.v1`
...
- Pack agent ID `video.distributor` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.delivery_qc_reviewer` is historical mapping only.
...
# Prompt — `video.prompt.distributor.v1`
...
You are **DistributorAgent (VA Domain Pack)** (`video.distributor`), a pack agent in the video domain swarm.
...
Design tool surface: Delivery management systems, outlet spec DB, packaging validators
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: SalesAgent, ArchiveMasterAgent, SoundMixerAgent, ColoristAgent
- May comment on: Spec mismatches, incomplete outlet packages, routing errors
...
Criteria (design): Outlet-spec compliance, handoff completeness, territorial routing accuracy
...
Execute the craft step for **DistributorAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.distributor`.
> Does **not** enable production models or network. Host `agent_spec.json` remains authoritative.

### Applied item ids
15, 21, 26, 30, 31, 37, 38, 59, 63, 69, 78, 87, 88, 93, 94
...
### `prompts/video.prompt.distributor.v1.md`
...
Source rubric `video.rubric.distributor.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.distributor.v1",
  "agent_id": "video.distributor",
  "title": "L2 craft rubric for DistributorAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Outlet-spec compliance, handoff completeness, territorial routing accuracy",
          "description": "Outlet-spec compliance, handoff completeness, territorial routing accuracy",
          "weight": 1.0,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Reduces delivery-spec mismatches relative to fragmented delivery ops",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Outlet-spec compliance, handoff completeness, territorial routing accuracy",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.distributor.v1.json`
...
# Source acquisition runbook — `video.distributor`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.distributor",
  "plan_id": "video.distributor.distill.v1",
  "inputs": [
    "src_1"
  ],
  "extractors": [
  ...
  "memory_namespace": "pack.video.video.distributor",
...
```json
{
  ...
  "agent_id": "video.distributor",
  "previous_common_agent_id": "video.delivery_qc_reviewer",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.distributor",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:10.803489Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:28.088835+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.distributor",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Distribution specs, outlet requirements, package handoff workflows",
      "description": "Distribution specs, outlet requirements, package handoff workflows",
