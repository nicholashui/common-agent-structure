...
> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.novelty/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.novelty`.
...
| **upstream_name** | NoveltyAgent / Anti-Cliché Critic |
| **category** | `9-Meta` |
| **domain_id** | `video` |
| **previous_common_id** | `video.judge_agent` |
| **status** | `registered` |
...
Host role binding: `NoveltyAgent / Anti-Cliché Critic (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
- Prompt reference: `video.prompt.novelty.v1`
- Rubric reference: `video.rubric.novelty.v1`
...
- Pack agent ID `video.novelty` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.judge_agent` is historical mapping only.
...
# Prompt — `video.prompt.novelty.v1`
...
You are **NoveltyAgent / Anti-Cliché Critic (VA Domain Pack)** (`video.novelty`), a pack agent in the video domain swarm.
...
Design tool surface: TV Tropes scraper; n-gram frequency DB; embedding novelty scorer
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: IdeationAgent, ScreenwriterAgent
- May comment on: ScreenwriterAgent (trope-stuffed), CopywriterAgent (templated)
...
Criteria (design): Cliché-hit count; novelty score vs category prior
...
Execute the craft step for **NoveltyAgent / Anti-Cliché Critic (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.novelty`.
> Does **not** enable production models or network. Host `agent_spec.json` remains authoritative.

### Applied item ids
10, 11, 15, 17, 20, 21, 26, 30, 31, 37, 38, 59, 63, 64, 86, 87, 88, 89, 90, 91, 93, 94, 95
...
### `prompts/video.prompt.novelty.v1.md`
...
Source rubric `video.rubric.novelty.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.novelty.v1",
  "agent_id": "video.novelty",
  "title": "L2 craft rubric for NoveltyAgent / Anti-Cliché Critic",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Cliché-hit count",
          "description": "Cliché-hit count",
          "weight": 0.5,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
        ...
        {
          "id": "d2",
          "name": "novelty score vs category prior",
          "description": "novelty score vs category prior",
          "weight": 0.5,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Catches more clichés than experienced script editor",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Cliché-hit count; novelty score vs category prior",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.novelty.v1.json`
...
# Source acquisition runbook — `video.novelty`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.novelty",
  "plan_id": "video.novelty.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.novelty",
...
```json
{
  ...
  "agent_id": "video.novelty",
  "previous_common_agent_id": "video.judge_agent",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.novelty",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:11.260951Z",
...
```json
{
  ...
  "sources": [
    ...
    {
      "id": "src_2",
      "title": "OpenSubtitles n-gram freq",
      "description": "OpenSubtitles n-gram freq",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
    ...
    {
      "id": "src_3",
      "title": "corpus-novelty embeddings",
      "description": "corpus-novelty embeddings",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
