...
> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.mpa/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.mpa`.
...
| **va_id** | 110 |
| **pack_id** | `video.mpa` |
| **upstream_name** | MPAAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.platform_policy_reviewer` |
| **status** | `registered` |
...
- Pack agent ID `video.mpa` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.platform_policy_reviewer` is historical mapping only.
...
|--------|------------|------|
| FilmAgent (2025) | Multi-agent film automation with iterative feedback loops that verify scripts and reduce hallucinations | [arXiv:2501.12909]([historical-url] |
| MovieAgent (2025) | Hierarchical CoT planning with character bank achieves SOTA script faithfulness and character consistency | [arXiv:2503.07314]([historical-url] |
| OmniAgent | Hierarchical graph-based multi-agent for long video with film-production-inspired architecture | [arXiv:2510.22431]([historical-url] |
| AnimAgents (2025) | Human-multi-agent collaboration with dedicated boards per pre-production stage | [arXiv:2511.17906]([historical-url] |
| Sima 1.0 (2025) | 11-step pipeline distributed across hybrid workforce for documentary video production | [arXiv:2604.07721]([historical-url] |
| Seedance 2.0 (Apr 2026) | 9 images + 3 videos + 3 audio simultaneous input; native audio-visual synchronization | [ByteDance]([historical-url] |
| Wan 2.6 (2026) | IP-anchored character consistency; multi-shot storytelling coherence | [Comparison]([historical-url] |
| Veo 3.1 (2026) | 4K + reference images for character/object direction; configurable aspect ratios | [Google AI]([historical-url] |
| Kling 2.6/3.0 | Physics-accurate motion; motion-control via reference video | [fal.ai]([historical-url] |
| Grok Imagine Video (xAI) | New entrant with strong image-to-video capability | [wavespeed.ai]([historical-url] |
| LangGraph 1.0 Production | Node caching, deferred nodes, pre/post hooks, consensus mechanisms | [LangChain]([historical-url] |
| Agent Architecture 2026 | Isolate orchestration from execution; event-driven avoids cascading failures | [markaicode]([historical-url] |
| Supervisor vs Swarm | Supervisor more accurate (routing is its only job); Swarm faster (skips intermediary) | [focused.io]([historical-url] |
| Generative UI 2026 | AI agents create rich interactive interfaces dynamically | [Medium]([historical-url] |
| 6-Model Comparison 2026 | Pick by goal: conversions, realism, camera control, storytelling, IP, or cost | [opencreator.io]([historical-url] |
...
Role-specific capability research for **MPAAgent** (`video.mpa`, va_id=110, category `10-Sup`).
...
You are **MPAAgent (VA Domain Pack)** (`video.mpa`), a pack agent in the video domain swarm.
...
Design tool surface: Submission packages, advisory templates, classification checklists
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: ProducerAgent / EP, LegalAgent, EthicsAgent
- May comment on: Missing advisories, incomplete rating prep, unclear classification support
...
Criteria (design): Rating-package completeness, advisory clarity, escalation quality
...
Execute the craft step for **MPAAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.mpa`.
> Does **not** enable production models or network. Host `agent_spec.json` remains authoritative.

### Applied item ids
15, 21, 26, 30, 31, 37, 38, 59, 63, 87, 88, 93, 94, 96
...
### `prompts/video.prompt.mpa.v1.md`
...
Source rubric `video.rubric.mpa.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.mpa.v1",
  "agent_id": "video.mpa",
  "title": "L2 craft rubric for MPAAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Rating-package completeness, advisory clarity, escalation quality",
          "description": "Rating-package completeness, advisory clarity, escalation quality",
          "weight": 1.0,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Prepares cleaner feature-release classification packages than manual prep",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Rating-package completeness, advisory clarity, escalation quality",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.mpa.v1.json`
...
# Source acquisition runbook — `video.mpa`
...
```json
{
  ...
  "memory_namespace": "pack.video.video.mpa",
...
### `sources/generic/video.mpa.SPEC.md`
...
```json
{
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.mpa",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Rating submission references, content advisories, theatrical packaging rules",
      "description": "Rating submission references, content advisories, theatrical packaging rules",
