# video.ugccreator — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.ugccreator/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.ugccreator`.
...
|-------|-------|
| **va_id** | 27 |
| **pack_id** | `video.ugccreator` |
| **upstream_name** | UGCCreatorAgent |
| **category** | `5-Perf` |
| **domain_id** | `video` |
| **previous_common_id** | `video.localization_qc_reviewer` |
| **status** | `registered` |
...
- Pack agent ID `video.ugccreator` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.localization_qc_reviewer` is historical mapping only.
...
- Master roster row va_id=27 from embedded agents.md content above.
...
<!-- self_contained_spec · video.ugccreator · va_id=27 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **UGCCreatorAgent** (`video.ugccreator`, va_id=27, category `5-Perf`).
...
<!-- migration_capability_research · video.ugccreator · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.ugccreator.v1`
...
You are **UGCCreatorAgent (VA Domain Pack)** (`video.ugccreator`), a pack agent in the video domain swarm.
...
Design tool surface: Veo 3.1 (portrait 9:16); ElevenLabs voice; CapCut API; TikTok Ads Manager
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: PerformanceMarketerAgent, BrandAgent
- May comment on: PerformanceMarketerAgent (wrong audience)
...
Criteria (design): Hook-rate ≥30%; "scripted" detector < threshold
...
Execute the craft step for **UGCCreatorAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.ugccreator`.
...
### `prompts/video.prompt.ugccreator.v1.md`
...
Source rubric `video.rubric.ugccreator.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.ugccreator.v1",
  "agent_id": "video.ugccreator",
  "title": "L2 craft rubric for UGCCreatorAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        ...
        {
          "id": "d2",
          "name": "\"scripted\" detector < threshold",
          "description": "\"scripted\" detector < threshold",
          "weight": 0.5,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Beats paid-creator avg ROAS at 0.1× cost",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Hook-rate ≥30%; \"scripted\" detector < threshold",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.ugccreator.v1.json`
...
# Source acquisition runbook — `video.ugccreator`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.ugccreator",
  "plan_id": "video.ugccreator.distill.v1",
  "inputs": [
    "src_1",
    "src_2"
  ],
  ...
  "memory_namespace": "pack.video.video.ugccreator",
...
```json
{
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.ugccreator",
...
```json
{
  ...
  "sources": [
    ...
    {
      "id": "src_2",
      "title": "Alix-Earle-style benchmarks (style not identity)",
      "description": "Alix-Earle-style benchmarks (style not identity)",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
