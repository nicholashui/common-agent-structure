# video.archiveresearch — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.archiveresearch/`.
...
# ArchiveResearchAgent

> Self-contained VA Domain Pack agent on host common-agent-swarm-ops.
> Pack agent ID matches pure VA/generic taxonomy: `video.archiveresearch`.
...
|-------|-------|
| **va_id** | 67 |
| **pack_id** | `video.archiveresearch` |
| **upstream_name** | ArchiveResearchAgent |
| **category** | `9-Meta` |
| **domain_id** | `video` |
| **previous_common_id** | `video.historical_replay_analyst` |
| **status** | `registered` |
...
Host role binding: `ArchiveResearchAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
Primary-source ratio; archive-coverage breadth

### Surpass-human signal (historical)

Higher primary-source ratio than doc producer
...
JSTOR/arXiv/PubMed APIs; Getty Images API; FOIA request tools; OCR (Tesseract)

**Runtime safety:** Host allow-lists are only `agent_spec.json` + `tool-permission-register.json`. CI uses video_* stubs. Do not treat design-time vendor names as enabled APIs.

### Architecture pattern (historical)

ReAct (formulate query → search archive → extract → grade source)
...
- Prompt reference: `video.prompt.archiveresearch.v1`
- Rubric reference: `video.rubric.archiveresearch.v1`
...
- Pack agent ID `video.archiveresearch` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.historical_replay_analyst` is historical mapping only.
...
- the noted technical stack and exact implementation parameters;
...
- `GROK_API_KEY` is missing for a noted LLM path;
- noted session or phase prerequisite files are missing;
...
- final report exists and contains noted sections;
...
- all noted artifacts are produced with the correct names and structures;
...
| 96 | **LMSAgent** | Packages and deploys learning content to LMS environments | SCORM/xAPI standards, LMS publishing workflows, completion-tracking schemas | Package va
…
...
- Master roster row va_id=67 from embedded agents.md content above.
...
<!-- self_contained_spec · video.archiveresearch · va_id=67 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **ArchiveResearchAgent** (`video.archiveresearch`, va_id=67, category `9-Meta`).
...
<!-- migration_capability_research · video.archiveresearch · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.archiveresearch.v1`
...
You are **ArchiveResearchAgent (VA Domain Pack)** (`video.archiveresearch`), a pack agent in the video domain swarm.
...
Design tool surface: JSTOR/arXiv/PubMed APIs; Getty Images API; FOIA request tools; OCR (Tesseract)
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: FactCheckerAgent, SMEAgent
- May comment on: ScriptwriterAgent (secondary-source reliance)
...
Criteria (design): Primary-source ratio; archive-coverage breadth
...
Execute the craft step for **ArchiveResearchAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.archiveresearch`.
...
### `prompts/video.prompt.archiveresearch.v1.md`
...
Source rubric `video.rubric.archiveresearch.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.archiveresearch.v1",
  "agent_id": "video.archiveresearch",
  "title": "L2 craft rubric for ArchiveResearchAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Primary-source ratio",
          "description": "Primary-source ratio",
          "weight": 0.5,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
        ...
        {
          "id": "d2",
          "name": "archive-coverage breadth",
          "description": "archive-coverage breadth",
          "weight": 0.5,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Higher primary-source ratio than doc producer",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Primary-source ratio; archive-coverage breadth",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.archiveresearch.v1.json`
...
# Source acquisition runbook — `video.archiveresearch`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.archiveresearch",
  "plan_id": "video.archiveresearch.distill.v1",
  "inputs": [
    "src_1"
  ],
  "extractors": [
  ...
  "memory_namespace": "pack.video.video.archiveresearch",
...
### `sources/generic/video.archiveresearch.SPEC.md`
...
```json
{
  ...
  "agent_id": "video.archiveresearch",
  "previous_common_agent_id": "video.historical_replay_analyst",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.archiveresearch",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:10.996985Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:27.653817+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.archiveresearch",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "JSTOR, arXiv, PubMed, AP Archive, Getty, FOIA",
      "description": "JSTOR, arXiv, PubMed, AP Archive, Getty, FOIA",
...
### `sources/study/research_agent_technical_specification.md`
...
…(clipped 34051 characters from `research_agent_technical_specification.md`)
