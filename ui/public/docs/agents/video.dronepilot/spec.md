# video.dronepilot — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.dronepilot/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.dronepilot`.
...
|-------|-------|
| **va_id** | 8 |
| **pack_id** | `video.dronepilot` |
| **upstream_name** | DronePilotAgent |
| **category** | `2-Cam` |
| **domain_id** | `video` |
| **previous_common_id** | `video.release_manager` |
| **status** | `registered` |
...
Host role binding: `DronePilotAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
Path smoothness; geofence compliance 100%; horizon stability

### Surpass-human signal (historical)

Competition-grade smoothness at 10× sortie rate; zero violations
...
DJI Waypoint SDK (sim); Veo 3.1 aerial-mode; geofence DB (AirMap API)

**Runtime safety:** Host allow-lists are only `agent_spec.json` + `tool-permission-register.json`. CI uses video_* stubs. Do not treat design-time vendor names as enabled APIs.

### Architecture pattern (historical)

Constitutional AI (safety constitution: FAA rules as principles)
...
- Prompt reference: `video.prompt.dronepilot.v1`
- Rubric reference: `video.rubric.dronepilot.v1`
...
- Pack agent ID `video.dronepilot` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.release_manager` is historical mapping only.
...
- Master roster row va_id=8 from embedded agents.md content above.
...
<!-- self_contained_spec · video.dronepilot · va_id=8 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **DronePilotAgent** (`video.dronepilot`, va_id=8, category `2-Cam`).
...
<!-- migration_capability_research · video.dronepilot · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.dronepilot.v1`
...
You are **DronePilotAgent (VA Domain Pack)** (`video.dronepilot`), a pack agent in the video domain swarm.
...
Primary distillation sources (design): Philip Bloom tutorials; FAA Part 107; SkyPixel award reels

## Developer

### Tools (allowlist intent)
Design tool surface: DJI Waypoint SDK (sim); Veo 3.1 aerial-mode; geofence DB (AirMap API)
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: DoPAgent, SafetyAgent
- May comment on: DoPAgent (impossible heights), SafetyAgent (risk)
...
Criteria (design): Path smoothness; geofence compliance 100%; horizon stability
...
Execute the craft step for **DronePilotAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.dronepilot`.
...
### `prompts/video.prompt.dronepilot.v1.md`
...
Source rubric `video.rubric.dronepilot.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.dronepilot.v1.json`
...
# Source acquisition runbook — `video.dronepilot`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.dronepilot",
  "plan_id": "video.dronepilot.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.dronepilot",
...
### `sources/generic/video.distributor.SPEC.md`
...
Outlet-spec compliance, handoff completeness, territorial routing accuracy

## Surpass-human signal

Reduces delivery-spec mismatches relative to fragmented delivery ops
...
Delivery management systems, outlet spec DB, packaging validators
...
- Master roster row va_id=112 from embedded agents.md content above.
- Deep/extra text from `business/video/corpus/` and `C:\Project\va-agent-swarm`.
- Generator: `scripts/business/enrich_video_agent_specs.py`.
- Upstream project name (historical only): va-agent-swarm.


<!-- self_contained_spec · video.distributor · va_id=112 -->

## Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **DistributorAgent** (`video.distributor`, va_id=112, category `10-Sup`).
...
<!-- migration_capability_research · video.distributor · v1 · 2026-07-13 -->
...
```json
{
  ...
  "agent_id": "video.dronepilot",
  "previous_common_agent_id": "video.release_manager",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.dronepilot",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:11.127113Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:28.095465+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.dronepilot",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Philip Bloom tutorials",
      "description": "Philip Bloom tutorials",
    ...
    {
      "id": "src_3",
      "title": "SkyPixel award reels",
      "description": "SkyPixel award reels",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
