...
> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.lms/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.lms`.
...
| **va_id** | 96 |
| **pack_id** | `video.lms` |
| **upstream_name** | LMSAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.incident_manager` |
| **status** | `registered` |
...
- Pack agent ID `video.lms` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.incident_manager` is historical mapping only.
...
You are **LMSAgent (VA Domain Pack)** (`video.lms`), a pack agent in the video domain swarm.
...
Design tool surface: LMS APIs, SCORM/xAPI validators, course packaging tools
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: InstructionalDesignAgent, AccessibilityAgent, LearnerSimAgent
- May comment on: Package compliance, tracking errors, learning-objective mismatch
...
Criteria (design): Package validity, tracking integrity, deploy success rate
...
Execute the craft step for **LMSAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.lms`.
...
### `prompts/video.prompt.lms.v1.md`
...
Source rubric `video.rubric.lms.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.lms.v1.json`
...
# Source acquisition runbook — `video.lms`
...
```json
{
  ...
  "memory_namespace": "pack.video.video.lms",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "SCORM/xAPI standards, LMS publishing workflows, completion-tracking schemas",
      "description": "SCORM/xAPI standards, LMS publishing workflows, completion-tracking schemas",
