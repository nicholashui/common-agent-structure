...
> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.ux/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.ux`.
...
| **va_id** | 90 |
| **pack_id** | `video.ux` |
| **upstream_name** | UXAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.delivery_packager` |
| **status** | `registered` |
...
Host role binding: `UXAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
Readability, friction-point detection, user-flow clarity

### Surpass-human signal (historical)

Flags user confusion earlier than launch-stage support teams
...
UX review checklists, session replay, readability tools

**Runtime safety:** Host allow-lists are only `agent_spec.json` + `tool-permission-register.json`. CI uses video_* stubs. Do not treat design-time vendor names as enabled APIs.

### Architecture pattern (historical)

LLM-as-Judge with UX rubric
...
- Prompt reference: `video.prompt.ux.v1`
- Rubric reference: `video.rubric.ux.v1`
...
- Pack agent ID `video.ux` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.delivery_packager` is historical mapping only.
...
- Master roster row va_id=90 from embedded agents.md content above.
...
<!-- self_contained_spec · video.ux · va_id=90 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **UXAgent** (`video.ux`, va_id=90, category `10-Sup`).
...
<!-- migration_capability_research · video.ux · v1 · 2026-07-13 -->
...
You are **UXAgent (VA Domain Pack)** (`video.ux`), a pack agent in the video domain swarm.
...
Design tool surface: UX review checklists, session replay, readability tools
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: TemplateDesignAgent, PersonalizationEngineerAgent, AccessibilityAgent
- May comment on: Confusing flows, readability issues, weak interaction cues
...
Criteria (design): Readability, friction-point detection, user-flow clarity
...
Execute the craft step for **UXAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.ux`.
...
Source rubric `video.rubric.ux.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.ux.v1.json`
...
# Source acquisition runbook — `video.ux`
...
```json
{
  ...
  "memory_namespace": "pack.video.video.ux",
...
```json
{
  ...
  "agent_id": "video.ux",
  "previous_common_agent_id": "video.delivery_packager",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.ux",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:11.357589Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:28.981145+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.ux",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "UX heuristics, accessibility criteria, usability testing patterns",
      "description": "UX heuristics, accessibility criteria, usability testing patterns",
