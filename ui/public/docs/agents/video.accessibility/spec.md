# video.accessibility — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.accessibility/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.accessibility`.
...
| **upstream_name** | AccessibilityAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.caption_specialist` |
| **status** | `registered` |
...
Host role binding: `AccessibilityAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
- Prompt reference: `video.prompt.accessibility.v1`
- Rubric reference: `video.rubric.accessibility.v1`
...
- Pack agent ID `video.accessibility` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.caption_specialist` is historical mapping only.
...
# Prompt — `video.prompt.accessibility.v1`
...
You are **AccessibilityAgent (VA Domain Pack)** (`video.accessibility`), a pack agent in the video domain swarm.
...
Design tool surface: Caption validators, contrast analyzers, AD review tools
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: AccessibilityOptimizerAgent, EditorAgent, ColoristAgent, SoundMixerAgent
- May comment on: Caption sync, contrast issues, missing AD or sign-language layers
...
Criteria (design): Caption accuracy, AD completeness, contrast compliance, release-readiness
...
Execute the craft step for **AccessibilityAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.accessibility`.
...
### `prompts/video.prompt.accessibility.v1.md`
...
Source rubric `video.rubric.accessibility.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.accessibility.v1",
  "agent_id": "video.accessibility",
  "title": "L2 craft rubric for AccessibilityAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Caption accuracy, AD completeness, contrast compliance, release-readiness",
          "description": "Caption accuracy, AD completeness, contrast compliance, release-readiness",
          "weight": 1.0,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Finds release-blocking accessibility issues before human audits do",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Caption accuracy, AD completeness, contrast compliance, release-readiness",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.accessibility.v1.json`
...
# Source acquisition runbook — `video.accessibility`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.accessibility",
  "plan_id": "video.accessibility.distill.v1",
  "inputs": [
    "src_1"
  ],
  "extractors": [
  ...
  "memory_namespace": "pack.video.video.accessibility",
...
```json
{
  ...
  "agent_id": "video.accessibility",
  "previous_common_agent_id": "video.caption_specialist",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.accessibility",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:10.706516Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:27.572982+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.accessibility",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "WCAG 2.2, captioning and AD guidelines, Deaf/HoH review frameworks",
      "description": "WCAG 2.2, captioning and AD guidelines, Deaf/HoH review frameworks",
