...
> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.comms/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.comms`.
...
| **va_id** | 104 |
| **pack_id** | `video.comms` |
| **upstream_name** | CommsAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.qc_l2_reviewer` |
| **status** | `registered` |
...
Host role binding: `CommsAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
- Prompt reference: `video.prompt.comms.v1`
- Rubric reference: `video.rubric.comms.v1`
...
- Pack agent ID `video.comms` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.qc_l2_reviewer` is historical mapping only.
...
You are **CommsAgent (VA Domain Pack)** (`video.comms`), a pack agent in the video domain swarm.
...
Design tool surface: Comms calendars, approval workflows, response templates
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: MarketingAgent, CommunityAgent, LegalAgent, BrandAgent
- May comment on: Disclosure gaps, inconsistent external messaging, weak response framing
...
Criteria (design): Message consistency, disclosure completeness, escalation quality
...
Execute the craft step for **CommsAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.comms`.
...
### `prompts/video.prompt.comms.v1.md`
...
Source rubric `video.rubric.comms.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.comms.v1",
  "agent_id": "video.comms",
  "title": "L2 craft rubric for CommsAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Message consistency, disclosure completeness, escalation quality",
          "description": "Message consistency, disclosure completeness, escalation quality",
          "weight": 1.0,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Produces faster aligned responses than fragmented stakeholder messaging",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Message consistency, disclosure completeness, escalation quality",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.comms.v1.json`
...
# Source acquisition runbook — `video.comms`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.comms",
  "plan_id": "video.comms.distill.v1",
  "inputs": [
    "src_1"
  ],
  "extractors": [
  ...
  "memory_namespace": "pack.video.video.comms",
...
```json
{
  ...
  "agent_id": "video.comms",
  "previous_common_agent_id": "video.qc_l2_reviewer",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.comms",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:11.085374Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:27.871901+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.comms",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Crisis communication guides, disclosure standards, PR playbooks",
      "description": "Crisis communication guides, disclosure standards, PR playbooks",
