# video.storyboard — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.storyboard/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.storyboard`.
...
| **upstream_name** | StoryboardAgent |
| **category** | `3-Edit` |
| **domain_id** | `video` |
| **previous_common_id** | `video.storyboard_artist` |
| **status** | `registered` |
...
- Pack agent ID `video.storyboard` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.storyboard_artist` is historical mapping only.
...
<!-- self_contained_spec · video.storyboard · va_id=14 -->
...
<!-- migration_capability_research · video.storyboard · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.storyboard.v1`
...
You are **StoryboardAgent (VA Domain Pack)** (`video.storyboard`), a pack agent in the video domain swarm.
...
Design tool surface: DALL-E 3 / Midjourney API; panel-layout templates; Fountain parser
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: DirectorAgent, DoPAgent
- May comment on: ScriptwriterAgent (unfilmable), DirectorAgent (staging)
...
Criteria (design): Shot-language fidelity; coverage completeness; staging clarity
...
Execute the craft step for **StoryboardAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.storyboard`.
...
### `prompts/video.prompt.storyboard.v1.md`
...
Source rubric `video.rubric.storyboard.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.storyboard.v1.json`
...
# Source acquisition runbook — `video.storyboard`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.storyboard",
  "plan_id": "video.storyboard.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.storyboard",
...
### `sources/generic/video.storyboard.SPEC.md`
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "*Framed Ink* (Mateu-Mestre)",
      "description": "*Framed Ink* (Mateu-Mestre)",
    ...
    {
      "id": "src_2",
      "title": "Pixar story-trust",
      "description": "Pixar story-trust",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
    ...
    {
      "id": "src_3",
      "title": "Despretz boards",
      "description": "Despretz boards",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
