# video.evaluationharness — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.evaluationharness/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.evaluationharness`.
...
| **upstream_name** | EvaluationHarnessAgent |
| **category** | `9-Meta` |
| **domain_id** | `video` |
| **previous_common_id** | `video.experiment_designer` |
| **status** | `registered` |
...
Host role binding: `EvaluationHarnessAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
- Prompt reference: `video.prompt.evaluationharness.v1`
- Rubric reference: `video.rubric.evaluationharness.v1`
...
- Pack agent ID `video.evaluationharness` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.experiment_designer` is historical mapping only.
...
# Prompt — `video.prompt.evaluationharness.v1`
...
You are **EvaluationHarnessAgent (VA Domain Pack)** (`video.evaluationharness`), a pack agent in the video domain swarm.
...
Design tool surface: VBench suite; EvalCrafter; MT-Bench harness; CI/CD (GitHub Actions); alerting (PagerDuty)
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: BenchmarkResearchAgent
- May comment on: All AI agents (regression alerts)
...
Criteria (design): Regression precision/recall; alert latency <1h
...
Execute the craft step for **EvaluationHarnessAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.evaluationharness`.
...
### `prompts/video.prompt.evaluationharness.v1.md`
...
Source rubric `video.rubric.evaluationharness.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.evaluationharness.v1.json`
...
# Source acquisition runbook — `video.evaluationharness`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.evaluationharness",
  "plan_id": "video.evaluationharness.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.evaluationharness",
...
```json
{
  ...
  "agent_id": "video.evaluationharness",
  "previous_common_agent_id": "video.experiment_designer",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.evaluationharness",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:10.919647Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:28.158916+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.evaluationharness",
...
```json
{
  ...
  "sources": [
    ...
    {
      "id": "src_3",
      "title": "benchmark repos",
      "description": "benchmark repos",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
