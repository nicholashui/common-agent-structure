# video.voiceover — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.voiceover/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.voiceover`.
...
|-------|-------|
| **va_id** | 21 |
| **pack_id** | `video.voiceover` |
| **upstream_name** | VoiceOverAgent |
| **category** | `4-Snd` |
| **domain_id** | `video` |
| **previous_common_id** | `video.voice_director` |
| **status** | `registered` |
...
Host role binding: `VoiceOverAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
Beats junior VO in blind preference; matches senior on emotion
...
ElevenLabs v3 TTS + voice cloning; Resemble.AI; pronunciation lexicon API

**Runtime safety:** Host allow-lists are only `agent_spec.json` + `tool-permission-register.json`. CI uses video_* stubs. Do not treat design-time vendor names as enabled APIs.

### Architecture pattern (historical)

LLM-as-Judge (MOS scoring rubric)
...
- Prompt reference: `video.prompt.voiceover.v1`
- Rubric reference: `video.rubric.voiceover.v1`
...
- Pack agent ID `video.voiceover` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.voice_director` is historical mapping only.
...
- Master roster row va_id=21 from embedded agents.md content above.
...
<!-- self_contained_spec · video.voiceover · va_id=21 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **VoiceOverAgent** (`video.voiceover`, va_id=21, category `4-Snd`).
...
<!-- migration_capability_research · video.voiceover · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.voiceover.v1`
...
You are **VoiceOverAgent (VA Domain Pack)** (`video.voiceover`), a pack agent in the video domain swarm.
...
Primary distillation sources (design): SOVAS reels; consented voice corpora; Wolfson/Cashman coaching

## Developer

### Tools (allowlist intent)
Design tool surface: ElevenLabs v3 TTS + voice cloning; Resemble.AI; pronunciation lexicon API
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: DirectorAgent, BrandAgent
- May comment on: ScriptwriterAgent (unspeakable phrasing)
...
Criteria (design): Prosody match; pronunciation 100%; emotion tag match
...
Execute the craft step for **VoiceOverAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.voiceover`.
...
### `prompts/video.prompt.voiceover.v1.md`
...
Source rubric `video.rubric.voiceover.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.voiceover.v1",
  "agent_id": "video.voiceover",
  "title": "L2 craft rubric for VoiceOverAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        ...
        {
          "id": "d2",
          "name": "pronunciation 100%",
          "description": "pronunciation 100%",
          "weight": 0.3333,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
        ...
        {
          "id": "d3",
          "name": "emotion tag match",
          "description": "emotion tag match",
          "weight": 0.3334,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Beats junior VO in blind preference; matches senior on emotion",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Prosody match; pronunciation 100%; emotion tag match",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.voiceover.v1.json`
...
# Source acquisition runbook — `video.voiceover`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.voiceover",
  "plan_id": "video.voiceover.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.voiceover",
...
### `sources/generic/video.voiceclone.SPEC.md`

# VoiceCloneAgent / LipSyncSpecialist
...
| # | Agent | Responsibility | Knowledge Distillation Source | Self-Quality Criteria | Surpass-Human Signal | Accepts Critique From / How | Comments On (Critiques) |
|---|-------|----------------|-------------------------------|-----------------------|----------------------|-----------------------------|--------------------------|
| 73 | **PromptOptimizerAgent** | Auto-improves prompts via OPRO / APE / DSPy / Promptbreeder | OPRO (Yang 2023), APE (Zhou 2022), DSPy (Stanford), Promptbreeder (DeepMind) | Score uplift per iteration on held-out eval; iteration count to convergence | Beats Karen X. Cheng / Paul Trillo
…
...
| 48 | **VoiceCloneAgent / LipSyncSpecialist** | Voice cloning + lip-sync | ElevenLabs safety docs; Wav2Lip/Sync.so; Baxter lip-sync refs | Voice MOS ≥4.2; phoneme-viseme error <40ms; consent verified | Wins blind MOS vs professional ADR | ComplianceAgent (consent), AnimatorAgent (lip-sync gold) | AvatarDesignAgent (face flicker), DubbingAgent | ElevenLabs v3 cloning API; Sync.so lip-sync; Wav2Lip; consent-doc verification | Self-Refine + MOS scoring model as judge |
| 49 | **AIQAConsistencyAgent** | Catches frame drift, hand/face artifacts, identity breaks | VBench; EvalCrafter; FVD literature; MPC/Weta QC checklists; deepfake models | Per-frame artifact score; identity-hash drift; hand/finger pass | Catches >95% of senior QC catches + 30% missed | DirectorAgent, VFXSupAgent | GeneratorAgent (re-roll), CompositorAgent | VBench evaluation suite; hand-detector models; face-ID embedding (ArcFace); frame-diff tools | Tool-use / ReAct (run detectors → flag → report) |
| 50 | **PersonalizationEngineerAgent** | Variable templates (name/face/voice swap) | Idomoo case studies; DMA campaigns; MarTech lit | Render-success ≥99.5%; spot
…
...
<!-- self_contained_spec · video.voiceclone · va_id=48 -->

## Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **VoiceCloneAgent / LipSyncSpecialist** (`video.voiceclone`, va_id=48, category `8-AI`).
...
<!-- migration_capability_research · video.voiceclone · v1 · 2026-07-13 -->

### `sources/generic/video.voiceover.SPEC.md`
...
```json
{
  ...
  "agent_id": "video.voiceover",
  "previous_common_agent_id": "video.voice_director",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.voiceover",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:10.593425Z",
...
```json
{
  ...
  "sources": [
    ...
    {
      "id": "src_2",
      "title": "consented voice corpora",
      "description": "consented voice corpora",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
    ...
    {
      "id": "src_3",
      "title": "Wolfson/Cashman coaching",
      "description": "Wolfson/Cashman coaching",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
