# video.screenwriter — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.screenwriter/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.screenwriter`.
...
| **upstream_name** | ScreenwriterAgent |
| **category** | `1-ATL` |
| **domain_id** | `video` |
| **previous_common_id** | `video.screenwriter` |
| **status** | `registered` |
...
- Pack agent ID `video.screenwriter` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.screenwriter` is historical mapping only.
...
# Prompt — `video.prompt.screenwriter.v1`
...
You are **ScreenwriterAgent (VA Domain Pack)** (`video.screenwriter`), a pack agent in the video domain swarm.
...
Primary distillation sources (design): Black List scripts; WGA library; McKee *Story*; Truby; Kaufman/Sorkin interviews

## Developer

### Tools (allowlist intent)
Design tool surface: Fountain/FDX format validators; semantic embedding models (text-embedding-3-large)
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: DirectorAgent, DramaturgAgent, StoryEditorAgent — Reflexion loop
- May comment on: DirectorAgent (logline), DialogueAgent, ConsistencyAgent
...
Criteria (design): Save-the-Cat beat pass; dialogue distinctiveness (embedding distance ≥τ); rewrite delta
...
Execute the craft step for **ScreenwriterAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.screenwriter`.
> Does **not** enable production models or network. Host `agent_spec.json` remains authoritative.

### Applied item ids
12, 15, 16, 21, 26, 30, 31, 32, 33, 37, 38, 42, 59, 63, 80, 87, 88, 93, 94
...
### `prompts/video.prompt.screenwriter.v1.md`
...
Source rubric `video.rubric.screenwriter.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.screenwriter.v1.json`
...
# Source acquisition runbook — `video.screenwriter`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.screenwriter",
  "plan_id": "video.screenwriter.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3",
  ...
  "memory_namespace": "pack.video.video.screenwriter",
...
```json
{
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.screenwriter",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Black List scripts",
      "description": "Black List scripts",
    ...
    {
      "id": "src_5",
      "title": "Kaufman/Sorkin interviews",
      "description": "Kaufman/Sorkin interviews",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
