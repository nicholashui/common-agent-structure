...
> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.analyst/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.analyst`.
...
| **upstream_name** | AnalystAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.analytics_reporter` |
| **status** | `registered` |
...
- Pack agent ID `video.analyst` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.analytics_reporter` is historical mapping only.
...
| Consistency | [arXiv 2510.21696 — BachVid]([historical-url] | Training-free consistency for background + character without reference image
…
...
# Prompt — `video.prompt.analyst.v1`
...
You are **AnalystAgent (VA Domain Pack)** (`video.analyst`), a pack agent in the video domain swarm.
...
Primary distillation sources (design): Platform analytics dashboards; experiment logs; evaluation-harness outputs; benchmark histories

## Developer

### Tools (allowlist intent)
Design tool surface: YouTube Analytics, Meta/TikTok Ads dashboards, BI warehouse, benchmark logs
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: SocialMediaStrategistAgent, PerformanceMarketerAgent, EvaluationHarnessAgent
- May comment on: Campaign pacing, release timing, retention and ROAS anomalies
...
Criteria (design): KPI completeness; forecast-vs-actual variance within tolerance; insight-to-action turnaround
...
Execute the craft step for **AnalystAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.analyst`.
...
### `prompts/video.prompt.analyst.v1.md`
...
Source rubric `video.rubric.analyst.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.analyst.v1.json`
...
# Source acquisition runbook — `video.analyst`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.analyst",
  "plan_id": "video.analyst.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3",
  ...
  "memory_namespace": "pack.video.video.analyst",
...
```json
{
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.analyst",
...
```json
{
  ...
  "sources": [
    ...
    {
      "id": "src_3",
      "title": "evaluation-harness outputs",
      "description": "evaluation-harness outputs",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
    ...
    {
      "id": "src_4",
      "title": "benchmark histories",
      "description": "benchmark histories",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
