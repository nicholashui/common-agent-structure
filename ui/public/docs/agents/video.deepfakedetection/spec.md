# video.deepfakedetection — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.deepfakedetection/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.deepfakedetection`.
...
| **upstream_name** | DeepfakeDetectionAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.c2pa_verifier` |
| **status** | `registered` |
...
- Pack agent ID `video.deepfakedetection` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.c2pa_verifier` is historical mapping only.
...
# Prompt — `video.prompt.deepfakedetection.v1`
...
You are **DeepfakeDetectionAgent (VA Domain Pack)** (`video.deepfakedetection`), a pack agent in the video domain swarm.
...
Design tool surface: Forensic models, face/voice anomaly detectors, provenance validators
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: AvatarDesignAgent, VoiceCloneAgent, TrustSafetyAgent, SafetyRedTeamAgent
- May comment on: Identity anomalies, provenance holes, deceptive synthesis patterns
...
Criteria (design): Forensic recall, false-negative control, provenance-validation accuracy
...
Execute the craft step for **DeepfakeDetectionAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.deepfakedetection`.
...
### `prompts/video.prompt.deepfakedetection.v1.md`
...
Source rubric `video.rubric.deepfakedetection.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.deepfakedetection.v1",
  "agent_id": "video.deepfakedetection",
  "title": "L2 craft rubric for DeepfakeDetectionAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Forensic recall, false-negative control, provenance-validation accuracy",
          "description": "Forensic recall, false-negative control, provenance-validation accuracy",
          "weight": 1.0,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Catches deceptive synthetic markers that generic QC misses",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Forensic recall, false-negative control, provenance-validation accuracy",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.deepfakedetection.v1.json`
...
# Source acquisition runbook — `video.deepfakedetection`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.deepfakedetection",
  "plan_id": "video.deepfakedetection.distill.v1",
  "inputs": [
    "src_1"
  ],
  "extractors": [
  ...
  "memory_namespace": "pack.video.video.deepfakedetection",
...
```json
{
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.deepfakedetection",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Deepfake forensics corpora, synthetic-media benchmarks, identity-risk studies",
      "description": "Deepfake forensics corpora, synthetic-media benchmarks, identity-risk studies",
