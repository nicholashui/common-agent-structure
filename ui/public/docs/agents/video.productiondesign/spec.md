# video.productiondesign — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.productiondesign/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.productiondesign`.
...
| **upstream_name** | ProductionDesignAgent |
| **category** | `3-Edit` |
| **domain_id** | `video` |
| **previous_common_id** | `video.production_designer` |
| **status** | `registered` |
...
- Pack agent ID `video.productiondesign` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.production_designer` is historical mapping only.
...
# Prompt — `video.prompt.productiondesign.v1`
...
You are **ProductionDesignAgent (VA Domain Pack)** (`video.productiondesign`), a pack agent in the video domain swarm.
...
Design tool surface: Unreal Engine (virtual scouting); Veo 3.1 location gen; archival image search APIs
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: DirectorAgent, DoPAgent
- May comment on: ConceptArtistAgent (style break), CostumeAgent
...
Criteria (design): Period accuracy; palette coherence; build feasibility
...
Execute the craft step for **ProductionDesignAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.productiondesign`.
...
### `prompts/video.prompt.productiondesign.v1.md`
...
Source rubric `video.rubric.productiondesign.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.productiondesign.v1.json`
...
# Source acquisition runbook — `video.productiondesign`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.productiondesign",
  "plan_id": "video.productiondesign.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.productiondesign",
...
```json
{
  ...
  "sources": [
    ...
    },
    {
      ...
      "origin": "spec/book_of_knowledge.md",
      "citation": "ZH: \u7535\u5f71\u7f8e\u672f\u8bbe\u8ba1\uff0cISBN-13 9787506492188"
    },
    {
      "kind": "reference_book",
      "language": "EN",
  ...
  "imported_from": "vendor/common-agent-swarm-ops/business/video/agents/video.productiondesign",
...
```json
{
  ...
  "sources": [
    ...
    {
      "id": "src_2",
      "title": "AMPAS submissions",
      "description": "AMPAS submissions",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
    ...
    {
      "id": "src_3",
      "title": "Beachler/Carter talks",
      "description": "Beachler/Carter talks",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
