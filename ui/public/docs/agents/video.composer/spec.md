...
> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.composer/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.composer`.
...
|-------|-------|
| **va_id** | 20 |
| **pack_id** | `video.composer` |
| **upstream_name** | ComposerAgent |
| **category** | `4-Snd` |
| **domain_id** | `video` |
| **previous_common_id** | `video.compositing_artist` |
| **status** | `registered` |
...
- Pack agent ID `video.composer` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.compositing_artist` is historical mapping only.
...
- Master roster row va_id=20 from embedded agents.md content above.
...
<!-- self_contained_spec · video.composer · va_id=20 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **ComposerAgent** (`video.composer`, va_id=20, category `4-Snd`).
...
<!-- migration_capability_research · video.composer · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.composer.v1`
...
You are **ComposerAgent (VA Domain Pack)** (`video.composer`), a pack agent in the video domain swarm.
...
Primary distillation sources (design): MAESTRO + film-score corpora; ASCAP/BMI; Zimmer/Hildur sessions

## Developer

### Tools (allowlist intent)
Design tool surface: Udio/Suno music gen API; MIDI toolchain; stem-separation (Demucs); loudness meter
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: DirectorAgent, EditorAgent (music cuts)
- May comment on: EditorAgent (cut interrupts cue), SoundDesignAgent (mask)
...
Criteria (design): Cue-to-emotion alignment (valence/arousal regression); thematic recurrence
...
Execute the craft step for **ComposerAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.composer`.
...
### `prompts/video.prompt.composer.v1.md`
...
Source rubric `video.rubric.composer.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.composer.v1.json`
...
# Source acquisition runbook — `video.composer`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.composer",
  "plan_id": "video.composer.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.composer",
...
### `sources/generic/video.vfxsupervisor.SPEC.md`
...
Shot-completion %; comp-error pixel count; CLIP-T vs plate

## Surpass-human signal

Weta-grade QC pass rate at fraction of time
...
Nuke via MCP bridge; Runway Gen-4 Aleph (video-to-video); ComfyUI

**Runtime safety:** Host allow-lists are only `agent_spec.json` + `tool-permission-register.json`. CI uses video_* stubs. Do not treat design-time vendor names as enabled APIs.

## Architecture pattern

Agentic Graph (fan-out per shot) + LLM-as-Judge (QC rubric)
...
<!-- self_contained_spec · video.vfxsupervisor · va_id=11 -->

## Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **VFXSupervisorAgent** (`video.vfxsupervisor`, va_id=11, category `3-Edit`).
...
<!-- migration_capability_research · video.vfxsupervisor · v1 · 2026-07-13 -->
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "MAESTRO + film-score corpora",
      "description": "MAESTRO + film-score corpora",
    ...
    {
      "id": "src_3",
      "title": "Zimmer/Hildur sessions",
      "description": "Zimmer/Hildur sessions",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
