# video.emotionalarc — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.emotionalarc/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.emotionalarc`.
...
| **upstream_name** | EmotionalArcAgent |
| **category** | `9-Meta` |
| **domain_id** | `video` |
| **previous_common_id** | `video.distribution_planner` |
| **status** | `registered` |
...
Host role binding: `EmotionalArcAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
- Prompt reference: `video.prompt.emotionalarc.v1`
- Rubric reference: `video.rubric.emotionalarc.v1`
...
- Pack agent ID `video.emotionalarc` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.distribution_planner` is historical mapping only.
...
# Prompt — `video.prompt.emotionalarc.v1`
...
You are **EmotionalArcAgent (VA Domain Pack)** (`video.emotionalarc`), a pack agent in the video domain swarm.
...
Design tool surface: Sentiment/emotion classifiers (GoEmotions); retention-curve predictor; biosignal proxy model
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: DirectorAgent, EditorAgent, ComposerAgent
- May comment on: EditorAgent (flat middle), ComposerAgent (cue mismatch)
...
Criteria (design): Curve-fit to target; biosignal-proxy regression accuracy
...
Execute the craft step for **EmotionalArcAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.emotionalarc`.
> Does **not** enable production models or network. Host `agent_spec.json` remains authoritative.

### Applied item ids
15, 21, 26, 30, 31, 37, 38, 49, 59, 63, 87, 88, 92, 93, 94
...
### `prompts/video.prompt.emotionalarc.v1.md`
...
Source rubric `video.rubric.emotionalarc.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.emotionalarc.v1",
  "agent_id": "video.emotionalarc",
  "title": "L2 craft rubric for EmotionalArcAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Curve-fit to target",
          "description": "Curve-fit to target",
          "weight": 0.5,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
        ...
        {
          "id": "d2",
          "name": "biosignal-proxy regression accuracy",
          "description": "biosignal-proxy regression accuracy",
          "weight": 0.5,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Better retention prediction than NRG test-screening cards",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Curve-fit to target; biosignal-proxy regression accuracy",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.emotionalarc.v1.json`
...
# Source acquisition runbook — `video.emotionalarc`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.emotionalarc",
  "plan_id": "video.emotionalarc.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.emotionalarc",
...
```json
{
  ...
  "agent_id": "video.emotionalarc",
  "previous_common_agent_id": "video.distribution_planner",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.emotionalarc",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:11.146464Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:28.132647+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "vendor/common-agent-swarm-ops/business/video/agents/video.emotionalarc",
...
```json
{
  ...
  "sources": [
    ...
    {
      "id": "src_2",
      "title": "affective-computing corpora",
      "description": "affective-computing corpora",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
    ...
    {
      "id": "src_3",
      "title": "Cron *Story Genius*",
      "description": "Cron *Story Genius*",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
