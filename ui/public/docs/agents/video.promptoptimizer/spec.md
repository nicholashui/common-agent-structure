# video.promptoptimizer — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.promptoptimizer/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.promptoptimizer`.
...
| **upstream_name** | PromptOptimizerAgent |
| **category** | `9-Meta` |
| **domain_id** | `video` |
| **previous_common_id** | `video.prompt_version_recorder` |
| **status** | `registered` |
...
Host role binding: `PromptOptimizerAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
- Prompt reference: `video.prompt.promptoptimizer.v1`
- Rubric reference: `video.rubric.promptoptimizer.v1`
...
- Pack agent ID `video.promptoptimizer` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.prompt_version_recorder` is historical mapping only.
...
# Prompt — `video.prompt.promptoptimizer.v1`
...
You are **PromptOptimizerAgent (VA Domain Pack)** (`video.promptoptimizer`), a pack agent in the video domain swarm.
...
Design tool surface: DSPy framework (MIPRO optimizer); OPRO implementation; held-out eval harness
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: PromptEngineerAgent, AIQAAgent
- May comment on: PromptEngineerAgent (sub-optimal seed)
...
Criteria (design): Score uplift per iteration; convergence speed
...
Execute the craft step for **PromptOptimizerAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.promptoptimizer`.
...
### `prompts/video.prompt.promptoptimizer.v1.md`
...
Source rubric `video.rubric.promptoptimizer.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.promptoptimizer.v1",
  "agent_id": "video.promptoptimizer",
  "title": "L2 craft rubric for PromptOptimizerAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Score uplift per iteration",
          "description": "Score uplift per iteration",
          "weight": 0.5,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Beats hand-tuned prompts on held-out briefs",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Score uplift per iteration; convergence speed",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.promptoptimizer.v1.json`
...
# Source acquisition runbook — `video.promptoptimizer`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.promptoptimizer",
  "plan_id": "video.promptoptimizer.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3",
  ...
  "memory_namespace": "pack.video.video.promptoptimizer",
...
```json
{
  ...
  "agent_id": "video.promptoptimizer",
  "previous_common_agent_id": "video.prompt_version_recorder",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.promptoptimizer",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:10.839785Z",
  "generic_source_depth": {
    "excerpts": true,
    "study": true,
    "adopted_at": "2026-07-27T01:30:28.684114+00:00",
    "upstream": "generic-swarm-ops/business/video/agents"
  },
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.promptoptimizer",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "OPRO (Yang 2023)",
      "description": "OPRO (Yang 2023)",
    ...
    {
      "id": "src_4",
      "title": "Promptbreeder (DeepMind)",
      "description": "Promptbreeder (DeepMind)",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
