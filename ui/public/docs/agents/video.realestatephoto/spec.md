# video.realestatephoto — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.realestatephoto/`.
...
# RealEstatePhotoAgent / 3D Scan

> Self-contained VA Domain Pack agent on host common-agent-swarm-ops.
> Pack agent ID matches pure VA/generic taxonomy: `video.realestatephoto`.
...
|-------|-------|
| **va_id** | 45 |
| **pack_id** | `video.realestatephoto` |
| **upstream_name** | RealEstatePhotoAgent / 3D Scan |
| **category** | `7-Edu` |
| **domain_id** | `video` |
| **previous_common_id** | `video.memory_curator` |
| **status** | `registered` |
...
- Pack agent ID `video.realestatephoto` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.memory_curator` is historical mapping only.
...
- Master roster row va_id=45 from embedded agents.md content above.
...
<!-- self_contained_spec · video.realestatephoto · va_id=45 -->

Migration capability research (v1 honest · 2026-07-13)

Role-specific capability research for **RealEstatePhotoAgent / 3D Scan** (`video.realestatephoto`, va_id=45, category `7-Edu`).
...
<!-- migration_capability_research · video.realestatephoto · v1 · 2026-07-13 -->
...
# Prompt — `video.prompt.realestatephoto.v1`
...
You are **RealEstatePhotoAgent / 3D Scan (VA Domain Pack)** (`video.realestatephoto`), a pack agent in the video domain swarm.
...
Design tool surface: Matterport SDK; HDR processing (Luminance HDR); lens-correction tools; Veo 3.1
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: DoPAgent, DronePilotAgent
- May comment on: DronePilotAgent (illegal altitude)
...
Criteria (design): Vertical-line straightness; HDR stack; coverage %
...
Execute the craft step for **RealEstatePhotoAgent / 3D Scan (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.realestatephoto`.
...
### `prompts/video.prompt.realestatephoto.v1.md`
...
Source rubric `video.rubric.realestatephoto.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.realestatephoto.v1",
  "agent_id": "video.realestatephoto",
  "title": "L2 craft rubric for RealEstatePhotoAgent / 3D Scan",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Vertical-line straightness",
          "description": "Vertical-line straightness",
          "weight": 0.3333,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Listing-CTR uplift vs human-shot baseline",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Vertical-line straightness; HDR stack; coverage %",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.realestatephoto.v1.json`
...
# Source acquisition runbook — `video.realestatephoto`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.realestatephoto",
  "plan_id": "video.realestatephoto.distill.v1",
  "inputs": [
    "src_1",
    "src_2"
  ],
  ...
  "memory_namespace": "pack.video.video.realestatephoto",
...
```json
{
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.realestatephoto",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Mike Kelley tutorials",
      "description": "Mike Kelley tutorials",
