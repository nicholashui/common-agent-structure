# video.musicvideodirector — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.musicvideodirector/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.musicvideodirector`.
...
| **upstream_name** | MusicVideoDirectorAgent |
| **category** | `5-Perf` |
| **domain_id** | `video` |
| **previous_common_id** | `video.critique_coordinator` |
| **status** | `registered` |
...
- Pack agent ID `video.musicvideodirector` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.critique_coordinator` is historical mapping only.
...
# Prompt — `video.prompt.musicvideodirector.v1`
...
You are **MusicVideoDirectorAgent (VA Domain Pack)** (`video.musicvideodirector`), a pack agent in the video domain swarm.
...
Primary distillation sources (design): DirectorsLibrary; UKMVA/MTV VMA winners; Hype Williams/Spike Jonze

## Developer

### Tools (allowlist intent)
Design tool surface: Runway Gen-4 (style-locked generation); Veo 3.1; mood-board tools (Are.na API)
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: LabelA&RAgent, ArtistAgent
- May comment on: EditorAgent (cut on beat), DoPAgent
...
Criteria (design): Edit-rhythm sync; lookbook coherence; artist-brief fit
...
Execute the craft step for **MusicVideoDirectorAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.musicvideodirector`.
...
### `prompts/video.prompt.musicvideodirector.v1.md`
...
Source rubric `video.rubric.musicvideodirector.v1.json` (baseline_safe; not a production pass).
...
### `rubrics/video.rubric.musicvideodirector.v1.json`
...
# Source acquisition runbook — `video.musicvideodirector`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.musicvideodirector",
  "plan_id": "video.musicvideodirector.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "owner": "video.musicvideodirector",
  "cadence": "quarterly",
  "promotion_criteria": [
  ...
  "memory_namespace": "pack.video.video.musicvideodirector",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "DirectorsLibrary",
      "description": "DirectorsLibrary",
    ...
    {
      "id": "src_2",
      "title": "UKMVA/MTV VMA winners",
      "description": "UKMVA/MTV VMA winners",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
    ...
    {
      "id": "src_3",
      "title": "Hype Williams/Spike Jonze",
      "description": "Hype Williams/Spike Jonze",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
