# video.learnersim — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.learnersim/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.learnersim`.
...
| **upstream_name** | LearnerSimAgent |
| **category** | `10-Sup` |
| **domain_id** | `video` |
| **previous_common_id** | `video.asset_security_officer` |
| **status** | `registered` |
...
Host role binding: `LearnerSimAgent (VA Domain Pack)`. Design-time VA table content below is historical and non-binding for activation.
...
- Prompt reference: `video.prompt.learnersim.v1`
- Rubric reference: `video.rubric.learnersim.v1`
...
- Pack agent ID `video.learnersim` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.asset_security_officer` is historical mapping only.
...
# Prompt — `video.prompt.learnersim.v1`
...
You are **LearnerSimAgent (VA Domain Pack)** (`video.learnersim`), a pack agent in the video domain swarm.
...
Design tool surface: Learner simulation models, assessment predictors, LMS data
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: InstructionalDesignAgent, LMSAgent, AnalystAgent
- May comment on: Confusing content, weak assessments, low-completion pathways
...
Criteria (design): Friction-point prediction, completion accuracy, simulated quiz realism
...
Execute the craft step for **LearnerSimAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.learnersim`.
...
### `prompts/video.prompt.learnersim.v1.md`
...
Source rubric `video.rubric.learnersim.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.learnersim.v1",
  "agent_id": "video.learnersim",
  "title": "L2 craft rubric for LearnerSimAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        {
          "id": "d1",
          "name": "Friction-point prediction, completion accuracy, simulated quiz realism",
          "description": "Friction-point prediction, completion accuracy, simulated quiz realism",
          "weight": 1.0,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Predicts weak spots before live learner complaints emerge",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "Friction-point prediction, completion accuracy, simulated quiz realism",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.learnersim.v1.json`
...
# Source acquisition runbook — `video.learnersim`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.learnersim",
  "plan_id": "video.learnersim.distill.v1",
  "inputs": [
    "src_1"
  ],
  "extractors": [
  ...
  "memory_namespace": "pack.video.video.learnersim",
...
```json
{
  ...
  "agent_id": "video.learnersim",
  "previous_common_agent_id": "video.asset_security_officer",
  "va_taxonomy_aligned": true,
  "generic_source": "generic-swarm-ops/business/video/agents/video.learnersim",
  "note": "Imported into CASOPS as baseline_safe. No production activation, no network, no plugins, memory writes forbidden.",
  "aligned_at": "2026-07-27T00:48:11.197894Z",
...
```json
{
  ...
  "sources": [
    {
      "id": "src_1",
      "title": "Learner-modeling datasets, completion analytics, quiz outcome patterns",
      "description": "Learner-modeling datasets, completion analytics, quiz outcome patterns",
