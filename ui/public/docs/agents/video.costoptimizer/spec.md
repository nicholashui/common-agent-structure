# video.costoptimizer — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.costoptimizer/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.costoptimizer`.
...
| **upstream_name** | CostOptimizerAgent |
| **category** | `9-Meta` |
| **domain_id** | `video` |
| **previous_common_id** | `video.cost_analyst` |
| **status** | `registered` |
...
- Pack agent ID `video.costoptimizer` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.cost_analyst` is historical mapping only.
...
# Prompt — `video.prompt.costoptimizer.v1`
...
You are **CostOptimizerAgent (VA Domain Pack)** (`video.costoptimizer`), a pack agent in the video domain swarm.
...
Primary distillation sources (design): Provider pricing; cost-quality frontiers; FrugalGPT patterns

## Developer

### Tools (allowlist intent)
Design tool surface: Provider pricing APIs; benchmark cost DB; FrugalGPT cascade logic
Runtime: only host-registered `allowed_tools` from agent_spec.json. Never invent credentials.

### Collaboration
- Accepts critique from: RouterAgent, FinanceAgent
- May comment on: RouterAgent (over-spend), GeneratorAgent (re-roll burn)
...
Criteria (design): $/successful-task; Pareto distance from frontier
...
Execute the craft step for **CostOptimizerAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.costoptimizer`.
...
### `prompts/video.prompt.costoptimizer.v1.md`
...
Source rubric `video.rubric.costoptimizer.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.costoptimizer.v1",
  "agent_id": "video.costoptimizer",
  "title": "L2 craft rubric for CostOptimizerAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L2_rubric": {
      ...
      "dimensions": [
        ...
        {
          "id": "d2",
          "name": "Pareto distance from frontier",
          "description": "Pareto distance from frontier",
          "weight": 0.5,
          "threshold_hint": null,
          "score_min": 0,
          "score_max": 100
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Lower $/quality than human CFO routing",
  ...
  "sources": {
    "agents_md_self_quality_criteria": "$/successful-task; Pareto distance from frontier",
    "research": [
      "LLM-as-Judge",
      "Self-Refine",
      "Constitutional AI"
...
### `rubrics/video.rubric.costoptimizer.v1.json`
...
# Source acquisition runbook — `video.costoptimizer`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.costoptimizer",
  "plan_id": "video.costoptimizer.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "memory_namespace": "pack.video.video.costoptimizer",
...
```json
{
  ...
  "sources": [
    ...
    {
      "id": "src_2",
      "title": "cost-quality frontiers",
      "description": "cost-quality frontiers",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
    ...
    {
      "id": "src_3",
      "title": "FrugalGPT patterns",
      "description": "FrugalGPT patterns",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
