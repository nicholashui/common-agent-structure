# video.performancemarketer — Spec

> Merged for Help from `SPEC.md`, `agent_spec.json`, `prompts/`, `rubrics/`, and `sources/` in `agents/video.performancemarketer/`.
...
> Pack agent ID matches pure VA/generic taxonomy: `video.performancemarketer`.
...
| **upstream_name** | PerformanceMarketerAgent |
| **category** | `6-Dist` |
| **domain_id** | `video` |
| **previous_common_id** | `video.performance_analyst` |
| **status** | `registered` |
...
- Pack agent ID `video.performancemarketer` is aligned to pure VA Domain Pack / generic pack taxonomy (Agent IDs ≈ VA tables).
- Previous common inventory ID `video.performance_analyst` is historical mapping only.
...
# Prompt — `video.prompt.performancemarketer.v1`
...
You are **PerformanceMarketerAgent (VA Domain Pack)** (`video.performancemarketer`), a pack agent in the video domain swarm.
...
Design tool surface: Meta Ads API; TikTok Ads API; Google Ads API; Bayesian AB testing libs
...
Execute the craft step for **PerformanceMarketerAgent (VA Domain Pack)**. Use the architecture pattern above (reason → optional tool calls → self-review → emit).
...
> Derived from `ui/RETHINK_100_IMPROVEMENTS.md` for `video.performancemarketer`.
...
### `prompts/video.prompt.performancemarketer.v1.md`
...
Source rubric `video.rubric.performancemarketer.v1.json` (baseline_safe; not a production pass).

```json
{
  "schema_version": "1.0",
  "rubric_id": "video.rubric.performancemarketer.v1",
  "agent_id": "video.performancemarketer",
  "title": "L2 craft rubric for PerformanceMarketerAgent",
  "pass_threshold": 85,
  "max_score": 100,
  "layers": {
    "L1_spec": {
    ...
    "L3_preference": {
      ...
      "surpass_signal_design": "Beats senior media buyer on 30-day ROAS",
...
### `rubrics/video.rubric.performancemarketer.v1.json`
...
# Source acquisition runbook — `video.performancemarketer`
...
```json
{
  "schema_version": "1.0",
  "agent_id": "video.performancemarketer",
  "plan_id": "video.performancemarketer.distill.v1",
  "inputs": [
    "src_1",
    "src_2",
    "src_3"
  ...
  "owner": "video.performancemarketer",
  "cadence": "quarterly",
  "promotion_criteria": [
  ...
  "memory_namespace": "pack.video.video.performancemarketer",
...
```json
{
  ...
  "imported_from": "C:\\Project\\common-agent-swarm-ops\\business\\video\\agents\\video.performancemarketer",
...
```json
{
  ...
  "sources": [
    ...
    {
      "id": "src_3",
      "title": "MMM literature",
      "description": "MMM literature",
      "license_class": "unknown_review_required",
      "acquisition_method": "manual_or_licensed_api",
      "local_path_hint": "sources/excerpts/ or sources/study/",
      "refresh_sla_days": 90,
