...
Do not dump the whole corpus. Route: metadata → cluster → Self-RAG reflection tokens (Asai et al., arXiv:2310.11511) → CRAG Correct/Incorrect/Ambiguous (Yan et al., arXiv:2401.15884). A route is wrong if the specialist still fails. This host has no live 5k-file index: route over local `sources/` and operator text; explain why. See `sources/study/domain_knowledge.md`.
...
Skill `casops.skill.router.slice-decision` is **declared** in `skills/bindings.json` and **not resolved**. Live enablement requires AND of author/inherited/operator_toggle **and** a host grant in `permissions/register.json`. Request: `skills/permission_request.json`.
