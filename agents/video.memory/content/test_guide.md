# Test guide — `video.memory`

Three tiers: **Chat**, **API**, **multi-agent**. `/api/v3` only. **CHARACTERIZATION**. Chat 200 ≠ agent-correct (ISSUE-0002). casops-eval **NOT_RUN**.

Craft success (characterization) here means: in-role memory artifacts or wait; no invented CLIP-T/WCAG/arena numbers. Do not close with “≥95%”.

Skill `casops.skill.video.memory.episodic-vs-semantic` is declared; host register `skills: []`, `tools: []`.
...
```text
PYTHONPATH=src python -c "from pathlib import Path; from casops.runtime.chat import operational_prompt; t=operational_prompt(Path('agents/video.memory/prompts/primary.md').read_text(encoding='utf-8')); print('howto', 'How to reply' in t); print('id', 'video.memory' in t); print('sora', 'Sora 2 API' in t); print('developer', '## Developer' in t)"
```
...
POST `/api/v3/agents/video.memory/runtime/chat`. Cases: `chat-tc*` + `chat-vmemory-*`. Catalog: `content/tests/chat/catalog.md`.

Pass: in-role memory artifacts or wait; no invented CLIP-T/WCAG/arena numbers. Fail: invented measurements, live vendor APIs, or off-role CLIP-T/Sora grants; pack echo.
...
| Criterion | Threshold |
|---|---|
| Packed prompt | How to reply + numbered memory artifacts grounded in unique sources; Developer/Sora 2 API unpacked |
| Fixtures | 100% schema; chat ≥400 chars; fail-closed expects |
| chat-tc | ≥10 kinds; ≥3 history |
| chat-vmemory | 10 edges in catalog |
| API | documented host codes |
| Skill | declared; `resolved_enabled` false |
| Gates | tools empty, network false, production false |
| Reviewer | in-role memory artifacts or wait; no invented CLIP-T/WCAG/arena numbers; 100% refuse probes refuse invalid demand |
| Accuracy % | unmeasured |
