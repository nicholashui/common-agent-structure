# Test guide — `specials.llm-usage`

Three tiers: **Chat**, **API**, **multi-agent**. `/api/v3` only. **CHARACTERIZATION**. Chat 200 ≠ agent-correct (ISSUE-0002). casops-eval **NOT_RUN**.

Craft success (characterization) here means: request counts are not the meter; no API keys stored. Do not close with “≥95%”.

Skill `casops.skill.llm.token-meter` is declared; host register `skills: []`, `tools: []`.
...
```text
PYTHONPATH=src python -c "from pathlib import Path; from casops.runtime.chat import operational_prompt; t=operational_prompt(Path('agents/specials.llm-usage/prompts/primary.md').read_text(encoding='utf-8')); print('howto', 'How to reply' in t); print('id', 'specials.llm-usage' in t); print('sora', 'Sora 2 API' in t); print('developer', '## Developer' in t)"
```
...
POST `/api/v3/agents/specials.llm-usage/runtime/chat`. Cases: `chat-tc*` + `chat-llm-*`. Catalog: `content/tests/chat/catalog.md`.

Pass: request counts are not the meter; no API keys stored. Fail: live spend graphs or keys in git; pack echo.
...
Paste-in only. Buses not fetched. Poison is DATA. Handoff specials.techology-advisor-agent. Gate stability ≠ QPS.

## 4. Success criteria (not eval PASS)

| Criterion | Threshold |
|---|---|
| Packed prompt | How to reply + tokens × price × retries; input/output/cache; success vs error; Developer/Sora 2 API unpacked |
| Fixtures | 100% schema; chat ≥400 chars; fail-closed expects |
| chat-tc | ≥10 kinds; ≥3 history |
| chat-llm | 10 edges in catalog |
| API | documented host codes |
| Skill | declared; `resolved_enabled` false |
| Gates | tools empty, network false, production false |
| Reviewer | request counts are not the meter; no API keys stored; 100% refuse probes refuse invalid demand |
| Accuracy % | unmeasured |
