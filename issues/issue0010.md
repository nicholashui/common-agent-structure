...
| `spec/wan_operation_guide.md` | `wan` | no (Wan 3.0 / Wan-Image guide; stub remains Wan 2.2 lineage) |
...
2. **Front-load the subject** (Grok / Seedance / GPT Image). First 20–30 words are identity. **Wan: event first, art second.**
3. **I2V: still carries look; motion describes change only** (Grok §9, Seedance §13, LTX I2V, Wan 3.0 first_frame). Do not rebuild the face in the motion prompt.
...
8. **Wan exclusivity:** `first_frame` / `last_frame` cannot mix with omni-reference. No negative-prompt box — exclusions stay in the main prompt. `prompt_extend` off when the prompt is exact. Not live.

## Work

- Grok compile: identity first; aspect/duration only in request fields; I2V motion uses hold-composition skeleton.
- Seedance / LTX / Hailuo / GPT Image / Wan: dialect `prompt.still` / `prompt.motion` for preview; `status: blocked`; `request: {}`.
