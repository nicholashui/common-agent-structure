...
def env_default_llm() -> str:
    value = (os.environ.get("DEFAULT_LLM") or "local_deterministic").strip()
    return value or "local_deterministic"
...
class LlmSettings:
    ...
    def load(cls, path: Path) -> "LlmSettings":
        ...
        cleaned = {str(key): str(value) for key, value in agents.items() if value}
        default = payload.get("default_llm")
        return cls(path=path, default_llm=str(default) if default else None, agents=cleaned)
    ...
    def resolved_default(self) -> str:
        return self.default_llm or env_default_llm()
...
class LlmRouter:
    ...
    def resolve(self, agent_id: str) -> str:
        provider = self.settings.resolved_for(agent_id)
    ...
    def complete(
        ...
        *,
        ...
        if provider == "local_deterministic":
            return self.local.complete(prompt=prompt, node_id=node_id, agent_id=agent_id)
        spec = PROVIDER_CATALOG[provider]
